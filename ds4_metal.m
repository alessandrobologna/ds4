...
typedef struct {
    ...
    uint64_t dst_head_stride;
    float    scale;
} ds4_gpu_dsv4_segmented_attention_args;
...
static int ds4_gpu_mul_ns(NSUInteger a, NSUInteger b, NSUInteger *out) {
    if (!out) return 0;
    if (a != 0 && b > NSUIntegerMax / a) return 0;
    *out = a * b;
    return 1;
}

static int ds4_gpu_add_ns(NSUInteger a, NSUInteger b, NSUInteger *out) {
    if (!out) return 0;
    if (b > NSUIntegerMax - a) return 0;
    *out = a + b;
    return 1;
}
...
int ds4_gpu_attention_segmented_mixed_heads_tensor(
    ...
    @autoreleasepool {
        ...
        id<MTLBuffer> qbuf = ds4_gpu_tensor_buffer(q);
        id<MTLBuffer> rawbuf = ds4_gpu_tensor_buffer(raw_kv);
        id<MTLBuffer> compbuf = comp_kv ? ds4_gpu_tensor_buffer(comp_kv) : rawbuf;
        id<MTLBuffer> topkbuf = topk ? ds4_gpu_tensor_buffer(topk) : rawbuf;
        id<MTLBuffer> rowsbuf = ds4_gpu_tensor_buffer(rows);
        id<MTLBuffer> headsbuf = ds4_gpu_tensor_buffer(heads);
        if (!qbuf || !rawbuf || !compbuf || !topkbuf || !rowsbuf || !headsbuf ||
            ds4_gpu_tensor_bytes(q) < q_bytes ||
            ds4_gpu_tensor_bytes(raw_kv) < raw_bytes ||
            (ratio != 0 && ds4_gpu_tensor_bytes(comp_kv) < comp_bytes) ||
            (top_k != 0 && ds4_gpu_tensor_bytes(topk) < topk_bytes) ||
            ds4_gpu_tensor_bytes(rows) < rows_bytes ||
            ds4_gpu_tensor_bytes(heads) < q_bytes) {
        ...
        id<MTLComputePipelineState> pipeline =
            ds4_gpu_get_pipeline("kernel_dsv4_segmented_mixed_attention_heads8_rb4");
        id<MTLComputePipelineState> sort_pipeline = nil;
        if (top_k != 0) {
            sort_pipeline = ds4_gpu_hot_pipeline(g_dsv4_sort_i32_rows_asc_pipeline,
                                                   "kernel_dsv4_sort_i32_rows_asc");
            ...
            if (!ds4_gpu_ensure_scratch_buffer(&g_indexed_topk_buffer,
                                                 &g_indexed_topk_bytes,
                                                 (NSUInteger)topk_bytes,
        ...
        ds4_gpu_dsv4_segmented_attention_args attn_args = {
            .n_tokens = n_tokens,
            .n_slots = n_slots,
            .n_head = n_head,
            .raw_cap = raw_cap,
        ...
        [enc setBuffer:qbuf offset:ds4_gpu_tensor_offset(q) atIndex:1];
        [enc setBuffer:rawbuf offset:ds4_gpu_tensor_offset(raw_kv) atIndex:2];
        [enc setBuffer:compbuf offset:comp_kv ? ds4_gpu_tensor_offset(comp_kv) : ds4_gpu_tensor_offset(raw_kv) atIndex:3];
        const NSUInteger topk_inner = top_k ? 0 :
            (topk ? ds4_gpu_tensor_offset(topk) : ds4_gpu_tensor_offset(raw_kv));
        [enc setBuffer:top_k ? g_indexed_topk_buffer : topkbuf
              offset:topk_inner
             atIndex:4];
        [enc setBuffer:sinks_buf offset:(NSUInteger)sinks_inner atIndex:5];
        [enc setBuffer:rowsbuf offset:ds4_gpu_tensor_offset(rows) atIndex:6];
        [enc setBuffer:headsbuf offset:ds4_gpu_tensor_offset(heads) atIndex:7];
        [enc setThreadgroupMemoryLength:4u * 128u * 4u * sizeof(float)
                                atIndex:0];
        [enc dispatchThreadgroups:MTLSizeMake((NSUInteger)n_tokens, ((NSUInteger)n_head + 7u) / 8u, 1)
             threadsPerThreadgroup:MTLSizeMake(32, 8, 1)];
        ds4_gpu_end_compute_encoder(cb, enc);

        if (!ds4_gpu_finish_command_buffer(cb, owned, "graph segmented mixed attention heads")) return 0;
    }
...
int ds4_gpu_attention_prefill_masked_raw_heads_tensor(
    ...
    @autoreleasepool {
        ...
        const NSUInteger mask_bytes =
            (NSUInteger)n_keys * (NSUInteger)n_tokens * sizeof(uint16_t);
        const NSUInteger kv_bytes = (NSUInteger)n_keys * row_bytes_f16;
        const NSUInteger pad_bytes = has_kvpad
            ? (NSUInteger)ncpsg * (2u * row_bytes_f16 + (NSUInteger)n_tokens * sizeof(uint16_t))
            : 1u;
        ...
        if (!ds4_gpu_finish_command_buffer(cb, owned,
                                           "graph masked raw prefill attention heads")) {
            return 0;
        }
    }
...
int ds4_gpu_attention_prefill_masked_multi_heads_tensor(
    ...
    @autoreleasepool {
        ...
        id<MTLBuffer> qbuf = ds4_gpu_tensor_buffer(q);
        id<MTLBuffer> kvbuf = ds4_gpu_tensor_buffer(packed_kv);
        id<MTLBuffer> headsbuf = ds4_gpu_tensor_buffer(heads);
        const uint64_t row_bytes = (uint64_t)head_dim * sizeof(float);
        const uint64_t q_bytes =
            (uint64_t)n_batches * n_tokens * n_head * row_bytes;
        const uint64_t kv_bytes_f32 =
            (uint64_t)n_batches * n_keys * row_bytes;
        if (!qbuf || !kvbuf || !headsbuf ||
            ds4_gpu_tensor_bytes(q) < q_bytes ||
            ds4_gpu_tensor_bytes(packed_kv) < kv_bytes_f32 ||
            ds4_gpu_tensor_bytes(heads) < q_bytes) {
        ...
        const bool bc_mask = (n_tokens % nqptg) != 0;
        const NSUInteger row_bytes_f16 = (NSUInteger)head_dim * sizeof(uint16_t);
        const NSUInteger one_mask_bytes =
            (NSUInteger)n_keys * (NSUInteger)n_tokens * sizeof(uint16_t);
        const NSUInteger mask_bytes = one_mask_bytes * (NSUInteger)n_batches;
        const NSUInteger kv_bytes =
            (NSUInteger)n_batches * (NSUInteger)n_keys * row_bytes_f16;
        const NSUInteger one_pad_bytes = (NSUInteger)ncpsg *
            (2u * row_bytes_f16 + (NSUInteger)n_tokens * sizeof(uint16_t));
        const NSUInteger pad_bytes =
            has_kvpad ? one_pad_bytes * (NSUInteger)n_batches : 1u;
        const NSUInteger nblk0 = ((NSUInteger)n_keys + ncpsg - 1u) / ncpsg;
        const NSUInteger nblk1 = ((NSUInteger)n_tokens + nqptg - 1u) / nqptg;
        const NSUInteger blk_bytes =
            ds4_gpu_align_up_ns(nblk0 * nblk1 * (NSUInteger)n_batches, 32u);

        id<MTLBuffer> mask_buffer =
            ds4_gpu_new_transient_buffer(mask_bytes, "ds4_flash_attn_multi_mask");
        if (!mask_buffer ||
            !ds4_gpu_ensure_scratch_buffer(&g_flash_attn_kv_buffer,
                                           &g_flash_attn_kv_bytes,
                                           kv_bytes,
        ...
        if (!ds4_gpu_encode_cpy_f32_f16_1d(cb,
                                           kvbuf,
                                           ds4_gpu_tensor_offset(packed_kv),
                                           g_flash_attn_kv_buffer,
                                           0,
        ...
        if (!ds4_gpu_finish_command_buffer(cb, owned,
                                           "graph multi masked prefill attention heads")) {
            return 0;
        }
    }
...
int ds4_gpu_attention_prefill_segmented_flash_heads_tensor(
    ...
    @autoreleasepool {
        ...
        const uint32_t ncpsg = 32;
        const uint32_t nwg = 32;
        const uint32_t nsg = ds4_gpu_flash_attn_vec_nsg(max_keys, nwg, ncpsg);
        ...
        if (!ds4_gpu_mul_ns((NSUInteger)n_packed_keys, (NSUInteger)row_bytes_f16, &kv_bytes) ||
            !ds4_gpu_mul_ns((NSUInteger)n_tokens, (NSUInteger)n_head, &nrows) ||
            !ds4_gpu_mul_ns(nrows, (NSUInteger)head_dim, &tmp_vec_bytes) ||
            !ds4_gpu_mul_ns(tmp_vec_bytes, (NSUInteger)nwg, &tmp_vec_bytes) ||
            !ds4_gpu_mul_ns(tmp_vec_bytes, sizeof(float), &tmp_vec_bytes) ||
            !ds4_gpu_mul_ns(2u, (NSUInteger)nwg, &reduce_width) ||
            !ds4_gpu_mul_ns(nrows, reduce_width, &tmp_reduce_bytes) ||
            !ds4_gpu_mul_ns(tmp_reduce_bytes, sizeof(float), &tmp_reduce_bytes) ||
            !ds4_gpu_add_ns(tmp_vec_bytes, tmp_reduce_bytes, &tmp_bytes)) {
            fprintf(stderr, "ds4: Metal segmented FlashAttention scratch size overflow\n");
            return 0;
        }
        ...
        id<MTLComputePipelineState> vec_pipeline =
            ds4_gpu_get_flash_attn_vec_pipeline("kernel_dsv4_segmented_flash_attn_ext_vec_f16_dk512_dv512",
        ...
        [enc setBuffer:g_flash_attn_kv_buffer offset:0 atIndex:2];
        [enc setBuffer:g_flash_attn_kv_buffer offset:0 atIndex:3];
        [enc setBuffer:rowsbuf offset:ds4_gpu_tensor_offset(rows) atIndex:4];
        [enc setBuffer:sinks_buf offset:(NSUInteger)sinks_inner atIndex:5];
        [enc setBuffer:g_flash_attn_pad_buffer offset:0 atIndex:6];
        [enc setBuffer:g_flash_attn_tmp_buffer offset:0 atIndex:7];
        [enc setThreadgroupMemoryLength:shared_bytes atIndex:0];
        ...
        if (!ds4_gpu_finish_command_buffer(cb, owned, "graph segmented FlashAttention heads")) return 0;
    }
