...
// Segmented prefill FlashAttention. Each query row carries metadata for the
// packed key span it may read, so unrelated prompt segments can share one
// vectorized dispatch without a dense block-diagonal CPU mask.
template<
    typename q4_t,
    typename k4_t,
    typename v4_t,
    ...
    short Q  = OP_FLASH_ATTN_EXT_VEC_NQPSG,
    short C  = OP_FLASH_ATTN_EXT_VEC_NCPSG>
kernel void kernel_dsv4_segmented_flash_attn_ext_vec(
        constant ds4_metal_args_flash_attn_ext_vec & args,
        device const char * q,
        device const char * k,
        device const char * v,
...
#define NS20 (FC_flash_attn_ext_vec_ns20)
    ...
    const ds4_metal_segmented_attention_row row = rows[iq1];

    {
        ...
        const short ikv2 = iq2/(args.ne02/args.ne_12_2);
        const short ikv3 = iq3/(args.ne03/args.ne_12_3);
        const uint64_t key_base = (uint64_t)row.key_offset*args.nb11;

        k += key_base + ikv2*args.nb12 + ikv3*args.nb13;
        v += key_base + ikv2*args.nb22 + ikv3*args.nb23;
    }
...

    {
        ...
        const uint32_t row_n_keys = row.n_raw + row.n_comp;
        const uint32_t row_keys_padded = row.n_keys_padded;

        for (int ic0 = iwg*NSG + sgitg; ; ic0 += NWG*NSG) {
            int ic = ic0*C;
            if ((uint32_t)ic >= row_keys_padded) {
                break;
            }

            const uint32_t local_key = (uint32_t)(ic + tiisg);
            bool allowed = false;
            if (local_key < row.n_raw) {
                const uint32_t key_pos = row.raw_first_pos + local_key;
                allowed = key_pos <= row.pos &&
                    (row.raw_window == 0 || row.pos - key_pos < row.raw_window);
            } else {
                const uint32_t comp_idx = local_key - row.n_raw;
                if (row.ratio != 0 && comp_idx < row.n_comp) {
                    const uint32_t visible = (row.pos + 1u) / row.ratio;
                    allowed = comp_idx < visible;
                }
            }
            if (local_key >= row_n_keys) {
                allowed = false;
            }
            sm[tiisg] = allowed ? half(0.0f) : half(-MAXHALF);
...
typedef decltype(kernel_dsv4_segmented_flash_attn_ext_vec<FA_TYPES, half4, 1, dequantize_f16_t4, half4, 1, dequantize_f16_t4, 128, 128, 4>) ds4_segmented_flash_attn_ext_vec_t;

template [[host_name("kernel_dsv4_segmented_flash_attn_ext_vec_f16_dk512_dv512")]]
kernel ds4_segmented_flash_attn_ext_vec_t kernel_dsv4_segmented_flash_attn_ext_vec<FA_TYPES, half4, 1, dequantize_f16_t4, half4, 1, dequantize_f16_t4, 512, 512, 1>;
