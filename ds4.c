...
static struct {
    ...
    uint32_t pos;
    uint32_t n_tokens;
    ds4_metal_stage_capture *capture;
} ds4_metal_stage_probe;

static void metal_graph_stage_probe_begin(ds4_metal_stage_capture *capture,
                                          uint32_t il,
                                          uint32_t pos,
                                          uint32_t n_tokens) {
    if (!capture) return;
    memset(capture, 0, sizeof(*capture));
    ds4_metal_stage_probe.active = true;
    ds4_metal_stage_probe.il = il;
    ds4_metal_stage_probe.pos = pos;
    ds4_metal_stage_probe.n_tokens = n_tokens;
    ds4_metal_stage_probe.capture = capture;
}

static void metal_graph_stage_probe_end(void) {
    ds4_metal_stage_probe.active = false;
    ds4_metal_stage_probe.capture = NULL;
}

static bool metal_graph_stage_probe_capture_tensor(const char *name,
                                                   ds4_metal_tensor *t,
                                                   uint64_t row_f32,
                                                   uint32_t il,
                                                   uint32_t pos,
                                                   uint32_t n_tokens) {
    ds4_metal_stage_capture *capture = ds4_metal_stage_probe.capture;
    if (!ds4_metal_stage_probe.active || !capture || !name || !t || row_f32 == 0) return true;
    if (ds4_metal_stage_probe.il != il ||
        ds4_metal_stage_probe.pos != pos ||
        ds4_metal_stage_probe.n_tokens != n_tokens)
    {
        return true;
    }
    if (capture->len >= (uint32_t)(sizeof(capture->items) / sizeof(capture->items[0]))) {
        capture->overflow = true;
        return true;
    }

    if (ds4_metal_synchronize() == 0) {
        fprintf(stderr, "ds4: failed to synchronize before stage probe %s layer %u pos %u n=%u\n",
                name, il, pos, n_tokens);
        return false;
    }

    const uint64_t bytes = row_f32 * sizeof(float);
    float *row = xmalloc((size_t)bytes);
    const bool ok = ds4_metal_tensor_read(t, 0, row, bytes) != 0;
    if (ok) {
        ds4_metal_stage_hash *item = &capture->items[capture->len++];
        item->name = name;
        item->n_f32 = row_f32;
    ...
    if (ds4_metal_begin_commands() == 0) {
        fprintf(stderr, "ds4: failed to resume Metal command batch after stage probe %s layer %u pos %u\n",
                name, il, pos);
        return false;
    }
    return ok;
...
static bool metal_graph_matmul_f16_maybe_rows(
        ...
        const ds4_metal_tensor *x,
        uint64_t                n_tokens);
static bool metal_graph_matmul_q8_0_maybe_rows(
        const ds4_metal_graph  *g,
        ds4_metal_tensor       *out,
        const ds4_model        *model,
        uint64_t                weight_offset,
...
static bool metal_graph_encode_output_head_batch(
    ...
    if (ok) ok = ds4_metal_rms_norm_plain_rows_tensor(g->batch_flat_hc,
                                                      ...
                                                      n_tokens,
                                                      DS4_RMS_EPS) != 0;
    if (ok) ok = metal_graph_matmul_f16_maybe_rows(g,
                                                   output_pre,
                                                   model,
                                                   weights->output_hc_fn->abs_offset,
                                                   hc_dim,
...
static bool metal_graph_encode_output_head_batch_row(
    ...
    const uint64_t hc_dim = (uint64_t)DS4_N_HC * DS4_N_EMBD;
    ds4_metal_tensor *row_hc = NULL;
    ds4_metal_tensor *logits = NULL;

    row_hc = ds4_metal_tensor_view(g->batch_cur_hc,
                                   (uint64_t)row * hc_dim * sizeof(float),
                                   hc_dim * sizeof(float));
    logits = ds4_metal_tensor_view(g->spec_logits,
                                   (uint64_t)row * vocab_dim * sizeof(float),
                                   vocab_dim * sizeof(float));
    bool ok = row_hc && logits;

    if (ok) ok = ds4_metal_rms_norm_plain_tensor(g->flat_hc,
                                                 row_hc,
                                                 (uint32_t)hc_dim,
                                                 DS4_RMS_EPS) != 0;
    if (ok) ok = ds4_metal_matmul_f16_tensor(g->output_pre,
    ...
    ds4_metal_tensor_free(logits);
    ds4_metal_tensor_free(row_hc);
    return ok;
}

static bool metal_graph_encode_output_head_batch_final(
        ds4_metal_graph *g,
        const ds4_model       *model,
        const ds4_weights     *weights,
        uint32_t               n_tokens,
    ...
    return metal_graph_encode_output_head_batch_row(g,
                                                    model,
                                                    weights,
                                                    n_tokens,
                                                    n_tokens - 1u,
...
static bool metal_graph_matmul_f16_maybe_rows(
    ...
    for (uint64_t t = 0; ok && t < n_tokens; t++) {
        ds4_metal_tensor *out_row = ds4_metal_tensor_view(out,
                                                          t * out_dim * sizeof(float),
                                                          out_dim * sizeof(float));
        ds4_metal_tensor *x_row = ds4_metal_tensor_view(x,
                                                        t * in_dim * sizeof(float),
                                                        in_dim * sizeof(float));
        ok = out_row && x_row &&
             ds4_metal_matmul_f16_tensor(out_row,
                                          model->map,
                                          model->size,
                                          weight_offset,
                                          in_dim,
...
static bool metal_graph_matmul_q8_0_maybe_rows(
    ...
    for (uint64_t t = 0; ok && t < n_tokens; t++) {
        ...
        ds4_metal_tensor *x_row = ds4_metal_tensor_view(x,
                                                        ...
                                                        in_dim * sizeof(float));
        ok = out_row && x_row &&
             ds4_metal_matmul_q8_0_tensor(out_row,
                                           model->map,
                                           model->size,
                                           weight_offset,
                                           in_dim,
...
static bool metal_graph_attention_output_q8_maybe_rows(
        const ds4_metal_graph  *g,
        ds4_metal_tensor       *out,
    ...
    for (uint32_t t = 0; ok && t < n_tokens; t++) {
        ds4_metal_tensor *out_row = ds4_metal_tensor_view(out,
                                                          ...
                                                          out_dim * sizeof(float));
        ds4_metal_tensor *low_row = ds4_metal_tensor_view(low,
                                                          (uint64_t)t * low_dim * sizeof(float),
                                                          low_dim * sizeof(float));
        ds4_metal_tensor *group_tmp_row = ds4_metal_tensor_view(group_tmp,
                                                                (uint64_t)t * group_dim * sizeof(float),
                                                                group_dim * sizeof(float));
        ds4_metal_tensor *low_tmp_row = ds4_metal_tensor_view(low_tmp,
                                                              (uint64_t)t * rank * sizeof(float),
                                                              rank * sizeof(float));
        ds4_metal_tensor *heads_row = ds4_metal_tensor_view(heads,
                                                            (uint64_t)t * heads_dim * sizeof(float),
                                                            heads_dim * sizeof(float));
        ok = out_row && low_row && group_tmp_row && low_tmp_row && heads_row &&
             ds4_metal_attention_output_q8_batch_tensor(out_row,
                                                        low_row,
                                                        group_tmp_row,
                                                        low_tmp_row,
                                                        model->map,
                                                        ...
                                                        heads_row,
                                                        1) != 0;
        ds4_metal_tensor_free(heads_row);
        ds4_metal_tensor_free(low_tmp_row);
        ds4_metal_tensor_free(group_tmp_row);
        ds4_metal_tensor_free(low_row);
        ds4_metal_tensor_free(out_row);
    }
    return ok;
}

static bool metal_graph_router_select_maybe_rows(
        const ds4_metal_graph  *g,
        ds4_metal_tensor       *selected,
        ds4_metal_tensor       *weights,
        ds4_metal_tensor       *probs,
        const ds4_model        *model,
        uint64_t                exp_probs_b_offset,
        uint64_t                tid2eid_offset,
        uint32_t                tid2eid_rows,
        uint32_t                token_offset,
        uint32_t                expert_offset,
        bool                    have_exp_probs_b,
        bool                    have_tid2eid,
    ...
    for (uint32_t t = 0; ok && t < n_tokens; t++) {
        ds4_metal_tensor *selected_row = ds4_metal_tensor_view(
                selected,
                (uint64_t)t * DS4_N_EXPERT_USED * sizeof(int),
                (uint64_t)DS4_N_EXPERT_USED * sizeof(int));
        ds4_metal_tensor *weights_row = ds4_metal_tensor_view(
                weights,
                (uint64_t)t * DS4_N_EXPERT_USED * sizeof(float),
                (uint64_t)DS4_N_EXPERT_USED * sizeof(float));
        ds4_metal_tensor *probs_row = ds4_metal_tensor_view(
                probs,
                (uint64_t)t * DS4_N_EXPERT * sizeof(float),
                (uint64_t)DS4_N_EXPERT * sizeof(float));
        ds4_metal_tensor *logits_row = ds4_metal_tensor_view(
                logits,
                (uint64_t)t * DS4_N_EXPERT * sizeof(float),
                (uint64_t)DS4_N_EXPERT * sizeof(float));
        ds4_metal_tensor *token_row = tokens ?
            ds4_metal_tensor_view(tokens, (uint64_t)t * sizeof(int32_t), sizeof(int32_t)) :
            NULL;
        ok = selected_row && weights_row && probs_row && logits_row &&
             (!tokens || token_row) &&
             ds4_metal_router_select_batch_tensor(selected_row,
                                                  weights_row,
                                                  probs_row,
                                                  model->map,
                                                  model->size,
                                                  ...
                                                  token_row,
                                                  1) != 0;
        ds4_metal_tensor_free(token_row);
        ds4_metal_tensor_free(logits_row);
        ds4_metal_tensor_free(probs_row);
        ds4_metal_tensor_free(weights_row);
        ds4_metal_tensor_free(selected_row);
    }
    return ok;
}
...
static bool metal_graph_refresh_ratio4_compressor_state(
    ...
    if (ok) {
        ok = metal_graph_matmul_f16_maybe_rows(g,
                                               g->batch_comp_kv,
                                               model,
...
#define DS4_METAL_STAGE_PROBE(name, tensor, row_f32) do { \
        if (ok) ok = metal_graph_stage_probe_capture_tensor((name), (tensor), (row_f32), il, pos0, n_tokens); \
    } while (0)
    if (ok) ok = ds4_metal_rms_norm_plain_rows_tensor(g->batch_flat_hc,
                                                      g->batch_cur_hc,
                                                      (uint32_t)hc_dim,
                                                      n_tokens,
                                                      DS4_RMS_EPS) != 0;
    DS4_METAL_STAGE_PROBE("hc_flat_norm", g->batch_flat_hc, hc_dim);
    if (ok) ok = metal_graph_matmul_f16_maybe_rows(g,
                                                   hc_mix_view,
                                                   model,
    ...
    DS4_METAL_STAGE_PROBE("hc_split", g->batch_hc_split, mix_hc);
    DS4_METAL_STAGE_PROBE("hc_attn_pre", g->batch_attn_cur, DS4_N_EMBD);
    if (ok) {
    ...
    if (ok) ok = metal_graph_matmul_q8_0_maybe_rows(g,
                                                    ...
                                                    g->batch_attn_norm,
                                                    n_tokens);
    DS4_METAL_STAGE_PROBE("q_lora", g->batch_qr, q_rank);
    if (ok) {
    ...
    if (qkv_rms_fused) {
        if (ok) ok = metal_graph_matmul_q8_0_maybe_rows(g,
                                                        ...
                                                        g->batch_attn_norm,
                                                        n_tokens);
        DS4_METAL_STAGE_PROBE("KVraw", g->batch_kv_raw, DS4_N_HEAD_DIM);
        if (ok) {
    ...
    DS4_METAL_STAGE_PROBE("q_lora_norm", g->batch_qr_norm, q_rank);
    if (qkv_rms_fused) {
        DS4_METAL_STAGE_PROBE("KVnorm", g->batch_kv, DS4_N_HEAD_DIM);
    }
    if (ok) {
    ...
    if (ok) ok = metal_graph_matmul_q8_0_maybe_rows(g,
                                                    ...
                                                    g->batch_qr_norm,
                                                    n_tokens);
    DS4_METAL_STAGE_PROBE("Qraw", g->batch_q, q_dim);
    if (ok) {
    ...
    if (ok) ok = ds4_metal_head_rms_norm_tensor(g->batch_q,
                                                ...
                                                DS4_N_HEAD_DIM,
                                                DS4_RMS_EPS) != 0;
    DS4_METAL_STAGE_PROBE("Qnorm", g->batch_q, q_dim);
    if (ok) {
    ...
    if (ok) ok = ds4_metal_rope_tail_tensor(g->batch_q,
                                            ...
                                            DS4_ROPE_YARN_BETA_FAST,
                                            DS4_ROPE_YARN_BETA_SLOW) != 0;
    DS4_METAL_STAGE_PROBE("Qcur", g->batch_q, q_dim);
    if (ok) {
    ...
    if (ok && raw_batch_attention) {
    ...
    } else if (ok && !spec_row_fallback && !zero_prefix && ratio == 0 && n_tokens <= g->raw_cap) {
        /*
        ...
        if (zero_prefix) {
            ...
            const bool aligned_chunk =
                !g->spec_verify_mode && (pos0 % ratio) == 0u && (n_tokens % ratio) == 0u;
            if (aligned_chunk) {
        ...
        if (ok && !spec_row_fallback && !zero_prefix && n_tokens <= g->raw_cap) {
            const uint32_t n_raw = metal_graph_raw_span_for_batch(g, pos0, n_tokens);
    ...
    DS4_METAL_STAGE_PROBE("kqv_out", g->batch_heads, q_dim);
    if (ok) ok = ds4_metal_rope_tail_tensor(g->batch_heads,
                                            ...
                                            DS4_ROPE_YARN_BETA_FAST,
                                            DS4_ROPE_YARN_BETA_SLOW) != 0;
    DS4_METAL_STAGE_PROBE("kqv_back", g->batch_heads, q_dim);
    if (ok) {
        metal_graph_debug_dump_tensor("kqv_back", g->batch_heads,
                                      (uint64_t)n_tokens * q_dim, il, pos0);
    }
    DS4_METAL_PROFILE_ATTN_STAGE("inv_rope");
    if (ok) ok = metal_graph_attention_output_q8_maybe_rows(g,
                                                            g->batch_attn_out,
                                                            g->batch_attn_low,
                                                            g->batch_group_tmp,
                                                            g->batch_low_tmp,
                                                            ...
                                                            g->batch_heads,
                                                            n_tokens);
    DS4_METAL_STAGE_PROBE("attn_low", g->batch_attn_low, (uint64_t)n_groups * rank);
    DS4_METAL_STAGE_PROBE("attn_out", g->batch_attn_out, DS4_N_EMBD);
    if (ok) {
    ...
    if (ok) ok = ds4_metal_hc_expand_split_tensor(after_attn_hc_view,
                                                  ...
                                                  DS4_N_EMBD,
                                                  DS4_N_HC) != 0;
    DS4_METAL_STAGE_PROBE("hc_attn_post", g->batch_after_attn_hc, hc_dim);
    if (ok) {
...
#define DS4_METAL_FFN_STAGE_PROBE(name, tensor, row_f32) do { \
        if (ok) ok = metal_graph_stage_probe_capture_tensor((name), (tensor), (row_f32), il, pos0, n_tokens); \
    } while (0)
    if (ok) ok = ds4_metal_rms_norm_plain_rows_tensor(g->batch_flat_hc,
                                                      ...
                                                      n_tokens,
                                                      DS4_RMS_EPS) != 0;
    DS4_METAL_FFN_STAGE_PROBE("ffn_hc_flat_norm", g->batch_flat_hc, hc_dim);
    if (ok) ok = metal_graph_matmul_f16_maybe_rows(g,
                                                   hc_mix_view,
                                                   model,
    ...
    DS4_METAL_FFN_STAGE_PROBE("ffn_hc_split", g->batch_hc_split, mix_hc);
    DS4_METAL_FFN_STAGE_PROBE("hc_ffn_pre", g->batch_ffn_cur, DS4_N_EMBD);
    if (ok) {
    ...
    if (ok) ok = metal_graph_matmul_f16_maybe_rows(g,
                                                   ...
                                                   g->batch_ffn_norm,
                                                   n_tokens);
    DS4_METAL_FFN_STAGE_PROBE("ffn_moe_logits", g->batch_router_logits, DS4_N_EXPERT);

    if (ok) ok = metal_graph_router_select_maybe_rows(g,
    ...
    if (ok) ok = ds4_metal_routed_moe_batch_tensor(g->batch_routed_out,
                                                   ...
                                                   g->batch_ffn_norm,
                                                   n_tokens) != 0;
    DS4_METAL_FFN_STAGE_PROBE("ffn_moe_gate", g->batch_routed_gate,
                              (uint64_t)DS4_N_EXPERT_USED * expert_mid_dim);
    DS4_METAL_FFN_STAGE_PROBE("ffn_moe_up", g->batch_routed_up,
                              (uint64_t)DS4_N_EXPERT_USED * expert_mid_dim);
    DS4_METAL_FFN_STAGE_PROBE("ffn_moe_mid", g->batch_routed_mid,
                              (uint64_t)DS4_N_EXPERT_USED * expert_mid_dim);
    DS4_METAL_FFN_STAGE_PROBE("ffn_moe_down", g->batch_routed_down,
                              (uint64_t)DS4_N_EXPERT_USED * DS4_N_EMBD);
    DS4_METAL_FFN_STAGE_PROBE("ffn_moe_out", g->batch_routed_out, DS4_N_EMBD);
    if (ok) {
    ...
    if (keep_ffn_out) {
        DS4_METAL_FFN_STAGE_PROBE("ffn_out", g->batch_ffn_out, DS4_N_EMBD);
    }
    if (ok && keep_ffn_out) {
...
#undef DS4_METAL_FFN_STAGE_PROBE
    return ok;
}
...
static bool metal_graph_verify_suffix_tops(
    ...
    if (ok) ok = metal_graph_upload_prompt_embeddings_hc(g->batch_cur_hc,
                                                         ...
                                                         start,
                                                         n_tokens);
    const double t_upload_done = profile ? now_sec() : 0.0;
    if (profile) profile->upload_sec = t_upload_done - t0;
    if (!ok) return false;
    ...
    ok = ds4_metal_begin_commands() != 0;
    if (ok) {
        if (want_tops || row_logits || getenv("DS4_MTP_SPEED_FULL_HEAD") != NULL) {
            ...
            ok = metal_graph_encode_output_head_batch_final(g,
                                                            model,
                                                            weights,
                                                            n_tokens,
                                                            weights->output->dim[1]);
    ...
    const double t_head_done = profile ? now_sec() : 0.0;
    if (profile) profile->head_sec = t_head_done - t_head0;
    const double t_top_read0 = profile ? now_sec() : 0.0;
    if (ok && want_tops) {
        ok = ds4_metal_tensor_read(g->comp_selected,
                                   0,
                                   row_tops,
    ...
    const double t_top_read_done = profile ? now_sec() : 0.0;
    if (profile) profile->top_read_sec = t_top_read_done - t_top_read0;
    const double t_logits_read0 = profile ? now_sec() : 0.0;
    if (ok && row_logits) {
    ...
    const double t_logits_read_done = profile ? now_sec() : 0.0;
    if (profile) {
        profile->logits_read_sec = t_logits_read_done - t_logits_read0;
        profile->total_sec = t_logits_read_done - t0;
    }
    return ok;
}
 ...
 *
 * It runs the target layers over the whole suffix so the trusted tail has a
 * continuation state, but computes target logits only for the requested early
 * validation rows and the final row.  The early rows validate draft[1..K];
 * when a check fails, the captured prefix-1 state lets the caller cheaply
 * commit only draft[0]. */
static bool metal_graph_verify_suffix_prefix_final(
        ds4_metal_graph *g,
        const ds4_model       *model,
        const ds4_weights     *weights,
        const token_vec       *prompt,
    ...
    ok = ds4_metal_begin_commands() != 0;
    for (uint32_t r = 0; ok && r < validate_rows; r++) {
    ...
    if (ok) ok = ds4_metal_indexer_topk_tensor(g->comp_selected,
                                               ...
                                               validate_rows,
                                               1) != 0;
    if (ok) ok = metal_graph_encode_output_head_batch_final(g,
                                                            model,
                                                            weights,
                                                            n_tokens,
                                                            weights->output->dim[1]);
    ...
    ok = ds4_metal_tensor_read(g->comp_selected,
                               0,
                               row_tops,
                               (uint64_t)validate_rows * sizeof(row_tops[0])) != 0;
    if (ok) ok = metal_graph_read_spec_logits_row(g, n_tokens - 1u, last_logits);
    return ok;
}
...
static bool metal_graph_verify_decode_exact_n(
        ds4_metal_graph *g,
        const ds4_model       *model,
        const ds4_weights     *weights,
        const int             *tokens,
    ...
    const uint64_t hc_dim = (uint64_t)DS4_N_HC * DS4_N_EMBD;
    ds4_metal_tensor *cur[4] = { NULL, NULL, NULL, NULL };
    ds4_metal_tensor *next[4] = { NULL, NULL, NULL, NULL };
    bool ok = true;
    for (uint32_t t = 0; ok && t < n_tokens; t++) {
        cur[t] = metal_graph_tensor_row_view(g->batch_cur_hc, t, hc_dim);
        next[t] = metal_graph_tensor_row_view(g->batch_next_hc, t, hc_dim);
        ok = cur[t] != NULL && next[t] != NULL;
    }

    for (uint32_t t = 0; ok && t < n_tokens; t++) {
        ok = ds4_metal_embed_token_hc_tensor(cur[t],
                                             model->map,
                                             model->size,
                                             weights->token_embd->abs_offset,
                                             (uint32_t)weights->token_embd->dim[1],
    ...
    for (uint32_t il = 0; ok && il < DS4_N_LAYER; il++) {
        for (uint32_t t = 0; ok && t < n_tokens; t++) {
            ...
            ok = metal_graph_encode_decode_layer(g,
                                                 ...
                                                 tokens[t]);
            if (ok && t == 0 && g->spec_capture_prefix1) {
                ok = metal_graph_capture_prefix1_attn_state(g, il) &&
                     metal_graph_capture_prefix1_index_state(g, il);
            }
        }
...
static DS4_MAYBE_UNUSED int logits_rank_for_token(const float *logits,
                                                  uint32_t n_vocab,
                                                  int token,
                                                  int max_rank) {
    if (!logits || token < 0 || token >= (int)n_vocab) return max_rank + 1;
    const float token_logit = logits[token];
    int rank = 1;
    for (uint32_t i = 0; i < n_vocab; i++) {
        if ((int)i == token) continue;
        if (logits[i] > token_logit && ++rank > max_rank) break;
    }
    return rank;
}
...
#define DS4_MTP_AUDIT_MAX_DEPTH 16
...
struct ds4_session {
    ...
    struct {
        ...
        uint64_t mtp_seq;
        uint64_t mtp_suffix_drafted;
        uint64_t mtp_suffix_committed;
        ...
        double mtp_verify_sec;
        double mtp_prefix_sec;
        double mtp_replay_sec;
        double mtp_total_sec;
    } mtp_stats;
    struct {
        uint64_t steps;
        ...
        uint64_t steps_with_mismatch;
        uint64_t steps_without_mismatch;
        uint64_t tokens_before_first_mismatch_sum;
        uint64_t position_total[DS4_MTP_AUDIT_MAX_DEPTH];
        uint64_t position_match[DS4_MTP_AUDIT_MAX_DEPTH];
        uint64_t depth_total[DS4_MTP_AUDIT_MAX_DEPTH + 1];
        uint64_t depth_match[DS4_MTP_AUDIT_MAX_DEPTH + 1];
        uint64_t target_margin_total[DS4_MTP_AUDIT_MARGIN_BUCKETS];
        uint64_t target_margin_match[DS4_MTP_AUDIT_MARGIN_BUCKETS];
        uint64_t mtp_margin_total[DS4_MTP_AUDIT_MARGIN_BUCKETS];
        uint64_t mtp_margin_match[DS4_MTP_AUDIT_MARGIN_BUCKETS];
        double exact_audit_sec;
    } mtp_speed_audit;
    struct {
        uint64_t steps;
        uint64_t failures;
...
typedef enum ds4_mtp_stats_path {
    DS4_MTP_STATS_FIRST_MISS,
...
static void ds4_mtp_stats_print(ds4_session *s) {
    ...
    const double target_ms =
        s->mtp_stats.target_eval_count ?
        (s->mtp_stats.target_eval_sec * 1000.0) / (double)s->mtp_stats.target_eval_count : 0.0;
    const double probe_ms =
        s->mtp_stats.mtp_probe_count ?
        (s->mtp_stats.mtp_probe_sec * 1000.0) / (double)s->mtp_stats.mtp_probe_count : 0.0;
    const double saved_ms = (double)s->mtp_stats.mtp_suffix_committed * target_ms;
    const double extra_ms = (s->mtp_stats.mtp_probe_sec +
                             s->mtp_stats.mtp_total_sec) * 1000.0;
    fprintf(stderr,
            ...
            (unsigned long long)s->mtp_stats.target_eval_count,
            target_ms,
            (unsigned long long)s->mtp_stats.mtp_probe_count,
            probe_ms,
            ...
            (unsigned long long)s->mtp_stats.mtp_suffix_drafted,
            (unsigned long long)s->mtp_stats.mtp_suffix_committed,
            (unsigned long long)s->mtp_stats.mtp_full_accept,
            (unsigned long long)s->mtp_stats.mtp_partial_accept,
            (unsigned long long)s->mtp_stats.mtp_adaptive_skip,
            s->mtp_stats.mtp_draft_sec * 1000.0,
            s->mtp_stats.mtp_verify_sec * 1000.0,
            s->mtp_stats.mtp_prefix_sec * 1000.0,
            s->mtp_stats.mtp_replay_sec * 1000.0,
            s->mtp_stats.mtp_total_sec * 1000.0,
            saved_ms,
            extra_ms,
            saved_ms - extra_ms);
...
static int ds4_mtp_audit_margin_bucket(float margin) {
    if (!isfinite(margin) || margin < 0.0f) return 0;
    if (margin < 0.1f) return 1;
    if (margin < 0.5f) return 2;
    if (margin < 1.0f) return 3;
    if (margin < 2.0f) return 4;
    if (margin < 5.0f) return 5;
    return 6;
}
...
static void ds4_mtp_speed_audit_print(ds4_session *s) {
    if (!s || !s->mtp_speed_audit_enabled || s->mtp_speed_audit.steps == 0) return;
    const uint64_t checked =
        s->mtp_speed_audit.exact_matches + s->mtp_speed_audit.exact_mismatches;
    const double match_rate =
        checked ? (100.0 * (double)s->mtp_speed_audit.exact_matches) / (double)checked : 0.0;
    const double avg_before_mismatch =
        s->mtp_speed_audit.steps ?
        (double)s->mtp_speed_audit.tokens_before_first_mismatch_sum /
            (double)s->mtp_speed_audit.steps : 0.0;
    const double avg_audit_ms =
        s->mtp_speed_audit.steps ?
        (s->mtp_speed_audit.exact_audit_sec * 1000.0) /
            (double)s->mtp_speed_audit.steps : 0.0;
    fprintf(stderr,
            "ds4: mtp speed-audit steps=%llu trusted=%llu exact_match=%llu/%llu "
            "match_rate=%.2f%% mismatch_steps=%llu clean_steps=%llu "
            "avg_tokens_before_mismatch=%.2f exact_audit=%.3fms avg=%.3fms\n",
            (unsigned long long)s->mtp_speed_audit.steps,
            (unsigned long long)s->mtp_speed_audit.trusted_tokens,
            (unsigned long long)s->mtp_speed_audit.exact_matches,
            (unsigned long long)checked,
            match_rate,
            (unsigned long long)s->mtp_speed_audit.steps_with_mismatch,
            (unsigned long long)s->mtp_speed_audit.steps_without_mismatch,
            avg_before_mismatch,
            s->mtp_speed_audit.exact_audit_sec * 1000.0,
            avg_audit_ms);

    fprintf(stderr, "ds4: mtp speed-audit by-position");
    for (int i = 0; i < DS4_MTP_AUDIT_MAX_DEPTH; i++) {
        const uint64_t total = s->mtp_speed_audit.position_total[i];
        if (!total) continue;
        const uint64_t match = s->mtp_speed_audit.position_match[i];
        fprintf(stderr,
                " p%d=%llu/%llu(%.1f%%)",
                i,
    ...
    fprintf(stderr, "ds4: mtp speed-audit by-draft-depth");
    for (int i = 1; i <= DS4_MTP_AUDIT_MAX_DEPTH; i++) {
        const uint64_t total = s->mtp_speed_audit.depth_total[i];
        if (!total) continue;
        const uint64_t match = s->mtp_speed_audit.depth_match[i];
        fprintf(stderr,
                " d%d=%llu/%llu(%.1f%%)",
                i,
                (unsigned long long)match,
    ...
    fprintf(stderr, "ds4: mtp speed-audit by-target-margin");
    for (int i = 0; i < DS4_MTP_AUDIT_MARGIN_BUCKETS; i++) {
        const uint64_t total = s->mtp_speed_audit.target_margin_total[i];
        if (!total) continue;
        const uint64_t match = s->mtp_speed_audit.target_margin_match[i];
        fprintf(stderr,
                " %s=%llu/%llu(%.1f%%)",
    ...
    fprintf(stderr, "ds4: mtp speed-audit by-mtp-margin");
    for (int i = 0; i < DS4_MTP_AUDIT_MARGIN_BUCKETS; i++) {
        const uint64_t total = s->mtp_speed_audit.mtp_margin_total[i];
        if (!total) continue;
        const uint64_t match = s->mtp_speed_audit.mtp_margin_match[i];
        fprintf(stderr,
                " %s=%llu/%llu(%.1f%%)",
                ds4_mtp_audit_margin_bucket_name(i),
                (unsigned long long)match,
...
static void ds4_mtp_tree_oracle_print(ds4_session *s) {
    if (!s || !s->mtp_tree_oracle_enabled || s->mtp_tree_oracle.steps == 0) return;
    ...
    for (int b = 0; b < DS4_MTP_TREE_K_BUCKETS; b++) {
        ...
        for (int d = 0; d < DS4_MTP_TREE_MAX_DEPTH; d++) {
            ...
            if (!total) continue;
            const uint64_t contained = s->mtp_tree_oracle.path_contains[b][d];
            fprintf(stderr,
                    " d%d=%llu/%llu(%.1f%%)",
                    d + 1,
    ...
    for (int d = 0; d < DS4_MTP_TREE_MAX_DEPTH; d++) {
        ...
        if (!total) continue;
        fprintf(stderr,
                "ds4: mtp tree-oracle pos%d total=%llu",
                d + 1,
                (unsigned long long)total);
        for (int b = 0; b < DS4_MTP_TREE_K_BUCKETS; b++) {
            const int k = ds4_mtp_tree_k_values[b];
            const uint64_t hit = s->mtp_tree_oracle.position_le[b][d];
            fprintf(stderr,
                    " <=top%d=%llu(%.1f%%)",
                    k,
...
static int ds4_mtp_speed_repeat_guard_cap(ds4_session *s, const int *drafts, int draft_n) {
    const char *env = getenv("DS4_MTP_SPEED_REPEAT_GUARD");
    if (!env || !env[0] || !s || !drafts || draft_n <= 0) return draft_n;

    int ngram = 3;
    char *end = NULL;
    long v = strtol(env, &end, 10);
    if (end != env && v >= 2 && v <= 8) ngram = (int)v;
    if (draft_n < ngram) return draft_n;

    int window = 64;
    const char *window_env = getenv("DS4_MTP_SPEED_REPEAT_WINDOW");
    if (window_env && window_env[0]) {
        end = NULL;
        v = strtol(window_env, &end, 10);
        if (end != window_env && v >= ngram && v <= 512) window = (int)v;
    }

    int tail_len = s->checkpoint.len < window ? s->checkpoint.len : window;
    const int total = tail_len + draft_n;
    int *seq = xmalloc((size_t)total * sizeof(seq[0]));
    for (int i = 0; i < tail_len; i++) {
        seq[i] = s->checkpoint.v[s->checkpoint.len - tail_len + i];
    }
    for (int i = 0; i < draft_n; i++) seq[tail_len + i] = drafts[i];

    int cap = draft_n;
    for (int end_pos = tail_len; end_pos < total; end_pos++) {
        const int start = end_pos - ngram + 1;
        if (start < 0) continue;
        for (int prev = 0; prev < start; prev++) {
            bool match = true;
            for (int j = 0; j < ngram; j++) {
                if (seq[prev + j] != seq[start + j]) {
                    match = false;
                    break;
                }
            }
...
static bool ds4_mtp_speed_candidate_repeats(ds4_session *s,
                                            const int *drafts,
                                            int draft_n,
                                            int candidate,
                                            int ngram,
                                            int window) {
    if (!s || !drafts || ngram < 2 || window < ngram) return false;
    const int tail_len = s->checkpoint.len < window ? s->checkpoint.len : window;
    const int total = tail_len + draft_n + 1;
    if (total < ngram) return false;
    ...
    for (int i = 0; i < draft_n; i++) seq[tail_len + i] = drafts[i];
    seq[total - 1] = candidate;

    bool repeats = false;
    const int start = total - ngram;
    for (int prev = 0; prev < start; prev++) {
        bool match = true;
        for (int j = 0; j < ngram; j++) {
            if (seq[prev + j] != seq[start + j]) {
...
static void ds4_mtp_speed_audit_check(ds4_session *s,
                                      const int *drafts,
                                      const float *mtp_margins,
                                      int draft_n,
                                      int trusted_n,
                                      uint32_t start) {
    if (!s || !s->engine || !s->mtp_speed_audit_enabled) return;
    if (!drafts || draft_n <= 0 || trusted_n <= 0) return;
    if (trusted_n > draft_n) trusted_n = draft_n;
    if (trusted_n > DS4_MTP_AUDIT_MAX_DEPTH) trusted_n = DS4_MTP_AUDIT_MAX_DEPTH;
    ...
    if (!ok) {
        fprintf(stderr, "ds4: mtp speed-audit failed to snapshot state\n");
        return;
    }
    ...
    int checked_n = 0;
    const int depth_bucket =
        draft_n <= DS4_MTP_AUDIT_MAX_DEPTH ? draft_n : DS4_MTP_AUDIT_MAX_DEPTH;

    for (int i = 0; i < trusted_n; i++) {
        int target_top = -1;
        float target_v0 = 0.0f;
        float target_v1 = 0.0f;
        logits_top2(s->logits,
                    DS4_N_VOCAB,
                    &target_top,
                    &target_v0,
                    NULL,
                    &target_v1);
        const bool match = target_top == drafts[i];
        if (match) matches++;
        else if (first_mismatch < 0) first_mismatch = i;
        checked_n++;

        s->mtp_speed_audit.position_total[i]++;
        if (match) s->mtp_speed_audit.position_match[i]++;
        s->mtp_speed_audit.depth_total[depth_bucket]++;
        if (match) s->mtp_speed_audit.depth_match[depth_bucket]++;

        const int target_bucket =
            ds4_mtp_audit_margin_bucket(target_v0 - target_v1);
        s->mtp_speed_audit.target_margin_total[target_bucket]++;
        if (match) s->mtp_speed_audit.target_margin_match[target_bucket]++;

        const float mtp_margin = mtp_margins ? mtp_margins[i] : -1.0f;
        const int mtp_bucket = ds4_mtp_audit_margin_bucket(mtp_margin);
        s->mtp_speed_audit.mtp_margin_total[mtp_bucket]++;
        if (match) s->mtp_speed_audit.mtp_margin_match[mtp_bucket]++;
        ...
        if (i + 1 < trusted_n) {
            ok = metal_graph_eval_token_raw_swa(&s->graph,
                                                ...
                                                s->logits);
            if (!ok) {
                fprintf(stderr, "ds4: mtp speed-audit exact decode failed at position %d\n", i);
                break;
            }
    ...
    s->mtp_speed_audit.steps++;
    s->mtp_speed_audit.trusted_tokens += (uint64_t)checked_n;
    s->mtp_speed_audit.exact_matches += matches;
    s->mtp_speed_audit.exact_mismatches += (uint64_t)checked_n - matches;
    if (first_mismatch >= 0) {
        s->mtp_speed_audit.steps_with_mismatch++;
        s->mtp_speed_audit.tokens_before_first_mismatch_sum += (uint64_t)first_mismatch;
    } else {
        s->mtp_speed_audit.steps_without_mismatch++;
        s->mtp_speed_audit.tokens_before_first_mismatch_sum += (uint64_t)checked_n;
    }
    s->mtp_speed_audit.exact_audit_sec += now_sec() - t0;
    ...
    if (!spec_frontier_restore(&frontier, s)) {
        fprintf(stderr, "ds4: mtp speed-audit failed to restore state\n");
...
static void ds4_mtp_tree_oracle_check(ds4_session *s, uint32_t start) {
    ...
    if (depth_env && depth_env[0]) {
        ...
        if (end != depth_env && v > 0 && v <= DS4_MTP_TREE_MAX_DEPTH) depth = (int)v;
    }
    if (depth <= 0) return;
    if (start + (uint32_t)depth >= (uint32_t)s->ctx_size) {
        depth = s->ctx_size > (int)start ? s->ctx_size - (int)start - 1 : 0;
        if (depth <= 0) return;
    }
    ...
    ds4_metal_tensor *saved_mtp_state = NULL;
    ds4_metal_tensor *saved_mtp_next = NULL;
    ds4_spec_frontier frontier;
    memset(&frontier, 0, sizeof(frontier));
    ...
    const bool saved_valid = s->checkpoint_valid;
    memcpy(saved_logits, s->logits, (size_t)DS4_N_VOCAB * sizeof(saved_logits[0]));
    memcpy(saved_mtp_logits, s->mtp_logits, (size_t)DS4_N_VOCAB * sizeof(saved_mtp_logits[0]));

    const double t0 = now_sec();
    bool ok = spec_frontier_snapshot(&frontier, s);
    bool have_frontier = ok;
    if (!ok) goto fail;
    saved_mtp_state = ds4_metal_tensor_alloc(hc_bytes);
    saved_mtp_next = ds4_metal_tensor_alloc(hc_bytes);
    if (!saved_mtp_state || !saved_mtp_next) goto fail;
    ok = ds4_metal_begin_commands() != 0;
    if (ok) ok = ds4_metal_tensor_copy(saved_mtp_state, 0, g->mtp_state_hc, 0, hc_bytes) != 0 &&
                 ds4_metal_tensor_copy(saved_mtp_next, 0, g->mtp_next_hc, 0, hc_bytes) != 0;
    if (ok) ok = ds4_metal_end_commands() != 0;
    else (void)ds4_metal_synchronize();
    if (!ok) goto fail;
    ...
    for (int i = 0; i < depth; i++) {
        const int top = sample_argmax(s->logits, DS4_N_VOCAB);
        target_path[i] = top;
        actual_depth++;
        if (top == ds4_token_eos(e)) break;
        if (i + 1 < depth) {
            ok = metal_graph_eval_token_raw_swa(g,
                                                &e->model,
                                                &e->weights,
                                                ...
                                                s->logits);
            if (!ok) goto fail;
            token_vec_push(&s->checkpoint, top);
        }
    }
    ...
    memcpy(s->logits, saved_logits, (size_t)DS4_N_VOCAB * sizeof(saved_logits[0]));
    if (!spec_frontier_restore(&frontier, s)) {
        fprintf(stderr, "ds4: mtp tree-oracle failed to restore state\n");
        s->checkpoint_valid = false;
        goto restore;
    }
    ...
    for (int d = 0; d < actual_depth; d++) {
        s->mtp_tree_oracle.position_total[d]++;
        for (int b = 0; b < DS4_MTP_TREE_K_BUCKETS; b++) {
            const int k = ds4_mtp_tree_k_values[b];
            if (ranks[d] <= k) s->mtp_tree_oracle.position_le[b][d]++;
            if (path_ok[b] && ranks[d] <= k) {
                s->mtp_tree_oracle.path_contains[b][d]++;
                path_len[b] = d + 1;
            } else {
                path_ok[b] = false;
            }
...
restore:
    if (have_frontier) g->mtp_n_raw = frontier.mtp_n_raw;
    if (saved_mtp_state && saved_mtp_next) {
        ok = ds4_metal_begin_commands() != 0;
        if (ok) ok = ds4_metal_tensor_copy(g->mtp_state_hc, 0, saved_mtp_state, 0, hc_bytes) != 0 &&
                     ds4_metal_tensor_copy(g->mtp_next_hc, 0, saved_mtp_next, 0, hc_bytes) != 0;
        if (ok) ok = ds4_metal_end_commands() != 0;
        else (void)ds4_metal_synchronize();
        if (!ok) {
            fprintf(stderr, "ds4: mtp tree-oracle failed to restore MTP state\n");
            s->checkpoint_valid = false;
        }
    }
    memcpy(s->mtp_logits, saved_mtp_logits, (size_t)DS4_N_VOCAB * sizeof(saved_mtp_logits[0]));
    ds4_metal_tensor_free(saved_mtp_next);
    ds4_metal_tensor_free(saved_mtp_state);
    spec_frontier_free(&frontier);
    free(saved_mtp_logits);
    free(saved_logits);
}

static void ds4_mtp_oracle_check_decode2(ds4_session *s,
                                         const int drafts[2],
                                         uint32_t start) {
    if (!s || !s->engine || s->mtp_oracle_reported) return;
    ds4_engine *e = s->engine;
    ds4_spec_frontier frontier;
    memset(&frontier, 0, sizeof(frontier));
    const int saved_len = s->checkpoint.len;
    bool ok = spec_frontier_snapshot(&frontier, s);
    if (!ok) {
        fprintf(stderr, "ds4: mtp oracle failed to snapshot state\n");
        s->mtp_oracle_reported = true;
        return;
    }

    float *saved_logits = xmalloc((size_t)DS4_N_VOCAB * sizeof(saved_logits[0]));
    float *seq0_logits = xmalloc((size_t)DS4_N_VOCAB * sizeof(seq0_logits[0]));
    float *seq1_logits = xmalloc((size_t)DS4_N_VOCAB * sizeof(seq1_logits[0]));
    float *exact0_logits = xmalloc((size_t)DS4_N_VOCAB * sizeof(exact0_logits[0]));
    float *exact1_logits = xmalloc((size_t)DS4_N_VOCAB * sizeof(exact1_logits[0]));
    float *fast_logits = xmalloc((size_t)2 * DS4_N_VOCAB * sizeof(fast_logits[0]));
    memcpy(saved_logits, s->logits, (size_t)DS4_N_VOCAB * sizeof(saved_logits[0]));

    bool seq_ok = false, exact_ok = false, fast_ok = false;
    int exact_top0 = -1;
    int fast_tops[2] = { -1, -1 };
    int seq_top0 = -1, seq_top1 = -1;
    int exact_logits_top0 = -1, exact_logits_top1 = -1;
    int fast_logits_top0 = -1, fast_logits_top1 = -1;

    seq_ok = metal_graph_eval_token_raw_swa(&s->graph,
                                            &e->model,
                                            &e->weights,
                                            drafts[0],
                                            start,
    ...
    if (seq_ok) {
        seq_top0 = sample_argmax(seq0_logits, DS4_N_VOCAB);
        seq_top1 = sample_argmax(seq1_logits, DS4_N_VOCAB);
    }
    ...
    exact_ok = metal_graph_verify_decode2_exact(&s->graph,
                                                &e->model,
                                                &e->weights,
                                                drafts[0],
                                                drafts[1],
                                                ...
                                                exact1_logits);
    if (exact_ok) {
        exact_logits_top0 = sample_argmax(exact0_logits, DS4_N_VOCAB);
        if (exact_top0 == drafts[1]) {
            exact_logits_top1 = sample_argmax(exact1_logits, DS4_N_VOCAB);
        }
    }
    ...
    fast_ok = metal_graph_verify_suffix_tops(&s->graph,
                                             &e->model,
                                             &e->weights,
                                             &s->checkpoint,
                                             start,
    ...
    s->checkpoint.len = saved_len;
    if (fast_ok) {
        fast_logits_top0 = sample_argmax(fast_logits, DS4_N_VOCAB);
        fast_logits_top1 = sample_argmax(fast_logits + DS4_N_VOCAB, DS4_N_VOCAB);
    }
    ...
    const bool diverged =
        !seq_ok || !exact_ok || !fast_ok ||
        exact_top0 != seq_top0 ||
        exact_logits_top0 != seq_top0 ||
        fast_tops[0] != seq_top0 ||
        fast_logits_top0 != seq_top0 ||
        fast_logits_top1 != seq_top1 ||
        (exact_top0 == drafts[1] && exact_logits_top1 != seq_top1);
    if (diverged || getenv("DS4_MTP_ORACLE_LOG_OK") != NULL) {
        const float fast0_delta = fast_ok && seq_ok ?
            max_abs_diff(seq0_logits, fast_logits, DS4_N_VOCAB) : -1.0f;
        const float fast1_delta = fast_ok && seq_ok ?
            max_abs_diff(seq1_logits, fast_logits + DS4_N_VOCAB, DS4_N_VOCAB) : -1.0f;
        const float exact0_delta = exact_ok && seq_ok ?
            max_abs_diff(seq0_logits, exact0_logits, DS4_N_VOCAB) : -1.0f;
        const float exact1_delta = (exact_ok && seq_ok && exact_top0 == drafts[1]) ?
            max_abs_diff(seq1_logits, exact1_logits, DS4_N_VOCAB) : -1.0f;
        fprintf(stderr,
                "ds4: mtp oracle %s pos=%u draft0=%d draft1=%d "
                "seq_ok=%d exact_ok=%d fast_ok=%d "
                "seq_top0=%d exact_top0=%d exact_logits_top0=%d fast_top0=%d fast_logits_top0=%d "
                "seq_top1=%d exact_logits_top1=%d fast_logits_top1=%d "
                "delta_exact0=%.6g delta_exact1=%.6g delta_fast0=%.6g delta_fast1=%.6g\n",
                diverged ? "divergence" : "ok",
                start,
                drafts[0],
                drafts[1],
                seq_ok ? 1 : 0,
                ...
                fast0_delta,
                fast1_delta);
        if (diverged) s->mtp_oracle_reported = true;
    }

done:
    s->checkpoint.len = saved_len;
    (void)spec_frontier_restore(&frontier, s);
    memcpy(s->logits, saved_logits, (size_t)DS4_N_VOCAB * sizeof(s->logits[0]));
    free(fast_logits);
    free(exact1_logits);
    free(exact0_logits);
    free(seq1_logits);
...
static bool metal_graph_probe_suffix_layer_hashes(ds4_metal_graph *g,
                                                  const ds4_model *model,
                                                  const ds4_weights *weights,
                                                  const token_vec *prompt,
                                                  uint32_t start,
                                                  uint32_t n_tokens,
                                                  uint64_t *attn_hashes,
                                                  uint64_t *ffn_hashes,
                                                  ds4_metal_stage_capture *stage_capture,
                                                  uint32_t stage_layer) {
    if (!g || !model || !weights || !prompt || !attn_hashes || !ffn_hashes ||
        n_tokens == 0 || n_tokens > g->prefill_cap ||
        start > (uint32_t)prompt->len || n_tokens > (uint32_t)prompt->len - start)
    {
        return false;
    }
    ...
    const uint64_t hc_dim = (uint64_t)DS4_N_HC * DS4_N_EMBD;
    const uint64_t hc_bytes = hc_dim * sizeof(float);
    float *row = xmalloc((size_t)hc_bytes);
    bool ok = metal_graph_upload_prompt_tokens(g->prefill_tokens, prompt, start, n_tokens);
    if (ok) ok = metal_graph_upload_prompt_embeddings_hc(g->batch_cur_hc,
                                                         g->prefill_tokens,
                                                         model,
    ...
    for (uint32_t il = 0; ok && il < DS4_N_LAYER; il++) {
        ok = ds4_metal_begin_commands() != 0;
        if (ok && stage_capture && il == stage_layer) {
            metal_graph_stage_probe_begin(stage_capture, il, start, n_tokens);
        }
        if (ok) ok = metal_graph_encode_layer_attention_batch(g,
                                                              model,
                                                              &weights->layer[il],
        ...
        if (!ok) {
            if (stage_capture && il == stage_layer) metal_graph_stage_probe_end();
            break;
        }
        ok = ds4_metal_tensor_read(g->batch_after_attn_hc, 0, row, hc_bytes) != 0;
        if (!ok) break;
        attn_hashes[il] = hash_bytes(row, hc_bytes);
        ...
        if (!ok) break;
        ok = ds4_metal_tensor_read(g->batch_next_hc, 0, row, hc_bytes) != 0;
        if (!ok) break;
        ffn_hashes[il] = hash_bytes(row, hc_bytes);
...
static void ds4_mtp_batch_probe_check(ds4_session *s,
                                      const int *drafts,
                                      int draft_n,
                                      uint32_t start) {
    if (!s || !s->engine || !drafts || draft_n < 2 || s->mtp_batch_probe_reported) return;
    const char *target_env = getenv("DS4_MTP_BATCH_PROBE_POS");
    if (target_env && target_env[0]) {
        char *end = NULL;
        unsigned long target = strtoul(target_env, &end, 10);
        if (end != target_env && start != (uint32_t)target) return;
    }
    const bool future_probe = getenv("DS4_MTP_BATCH_FUTURE_PROBE") != NULL;
    const bool n1_probe = getenv("DS4_MTP_BATCH_N1_N2_PROBE") != NULL;
    const bool prefix_probe = getenv("DS4_MTP_BATCH_PREFIX1_PROBE") != NULL;
    if (!future_probe && !n1_probe && !prefix_probe) return;

    s->mtp_batch_probe_reported = true;
    ds4_engine *e = s->engine;
    ds4_spec_frontier frontier;
    memset(&frontier, 0, sizeof(frontier));
    const int saved_len = s->checkpoint.len;
    ...
    if (!have_frontier) {
        fprintf(stderr, "ds4: mtp batch-probe failed to snapshot state\n");
        return;
    }

    float *saved_logits = xmalloc((size_t)DS4_N_VOCAB * sizeof(saved_logits[0]));
    float *seq0_logits = xmalloc((size_t)DS4_N_VOCAB * sizeof(seq0_logits[0]));
    float *seq_next_logits = xmalloc((size_t)DS4_N_VOCAB * sizeof(seq_next_logits[0]));
    float *n1_logits = xmalloc((size_t)DS4_N_VOCAB * sizeof(n1_logits[0]));
    float *fast_logits = xmalloc((size_t)2 * DS4_N_VOCAB * sizeof(fast_logits[0]));
    float *base_fast0_logits = xmalloc((size_t)DS4_N_VOCAB * sizeof(base_fast0_logits[0]));
    float *prefix_next_logits = xmalloc((size_t)DS4_N_VOCAB * sizeof(prefix_next_logits[0]));
    memcpy(saved_logits, s->logits, (size_t)DS4_N_VOCAB * sizeof(saved_logits[0]));
    memset(base_fast0_logits, 0, (size_t)DS4_N_VOCAB * sizeof(base_fast0_logits[0]));

    bool seq_ok = metal_graph_eval_token_raw_swa(&s->graph,
                                                 &e->model,
                                                 &e->weights,
                                                 drafts[0],
                                                 start,
    ...
    int seq_next_top = -1;
    if (seq_ok) {
        seq0_top = sample_argmax(seq0_logits, DS4_N_VOCAB);
        token_vec_push(&s->checkpoint, drafts[0]);
        if (seq0_top >= 0) {
            seq_ok = metal_graph_eval_token_raw_swa(&s->graph,
                                                    &e->model,
                                                    ...
                                                    start + 1u,
                                                    seq_next_logits);
            if (seq_ok) seq_next_top = sample_argmax(seq_next_logits, DS4_N_VOCAB);
        }
    }
    s->checkpoint.len = saved_len;
    memcpy(s->logits, saved_logits, (size_t)DS4_N_VOCAB * sizeof(saved_logits[0]));
    bool ok = spec_frontier_restore(&frontier, s);
    if (!ok) goto done;

    if (n1_probe) {
        int n1_top = -1;
        token_vec_push(&s->checkpoint, drafts[0]);
        const bool n1_ok = metal_graph_verify_suffix_tops(&s->graph,
                                                          &e->model,
                                                          &e->weights,
                                                          &s->checkpoint,
                                                          start,
                                                          ...
                                                          NULL);
        s->checkpoint.len = saved_len;
        const float n1_delta = n1_ok && seq_ok ?
            max_abs_diff(seq0_logits, n1_logits, DS4_N_VOCAB) : -1.0f;
        if (n1_ok) n1_top = sample_argmax(n1_logits, DS4_N_VOCAB);
        fprintf(stderr,
                "ds4: mtp batch-probe n1 pos=%u draft0=%d seq_ok=%d n1_ok=%d "
                "seq0_top=%d n1_top=%d delta=%.6g\n",
                start,
                drafts[0],
                seq_ok ? 1 : 0,
                n1_ok ? 1 : 0,
    ...
    int variants[4];
    const char *labels[4] = { "draft1", "seq0", "alt1", "alt2" };
    variants[0] = drafts[1];
    variants[1] = seq0_top >= 0 ? seq0_top : drafts[1];
    variants[2] = (drafts[1] + 9973) % DS4_N_VOCAB;
    variants[3] = (drafts[1] + 27183) % DS4_N_VOCAB;
    for (int i = 2; i < 4; i++) {
        if (variants[i] == variants[0] || variants[i] == variants[1]) {
            variants[i] = (variants[i] + 1) % DS4_N_VOCAB;
        }
    }

    for (int v = 0; v < 4; v++) {
        if (!future_probe && !prefix_probe) break;
        int row_tops[2] = { -1, -1 };
        token_vec_push(&s->checkpoint, drafts[0]);
        token_vec_push(&s->checkpoint, variants[v]);
        const bool fast_ok = metal_graph_verify_suffix_tops(&s->graph,
                                                            &e->model,
                                                            &e->weights,
                                                            &s->checkpoint,
                                                            start,
                                                            ...
                                                            NULL);
        s->checkpoint.len = saved_len;
        const int fast0_top = fast_ok ? sample_argmax(fast_logits, DS4_N_VOCAB) : -1;
        const int fast1_top = fast_ok ? sample_argmax(fast_logits + DS4_N_VOCAB, DS4_N_VOCAB) : -1;
        float delta_seq0 = -1.0f;
        float delta_base0 = -1.0f;
        if (fast_ok && seq_ok) delta_seq0 = max_abs_diff(seq0_logits, fast_logits, DS4_N_VOCAB);
        if (fast_ok) {
            if (v == 0) {
                memcpy(base_fast0_logits, fast_logits, (size_t)DS4_N_VOCAB * sizeof(fast_logits[0]));
                delta_base0 = 0.0f;
            } else {
                delta_base0 = max_abs_diff(base_fast0_logits, fast_logits, DS4_N_VOCAB);
            }
        }

        int prefix_next_top = -1;
        float prefix_next_delta = -1.0f;
        bool prefix_ok = false;
        if (prefix_probe && fast_ok && seq_ok && seq0_top >= 0) {
            prefix_ok = spec_frontier_commit_prefix1(s);
            if (prefix_ok) {
                prefix_ok = metal_graph_eval_token_raw_swa(&s->graph,
                                                           &e->model,
                                                           &e->weights,
                                                           seq0_top,
                                                           start + 1u,
            ...
            if (prefix_ok) {
                prefix_next_top = sample_argmax(prefix_next_logits, DS4_N_VOCAB);
                prefix_next_delta = max_abs_diff(seq_next_logits,
                                                 prefix_next_logits,
                                                 DS4_N_VOCAB);
            }
        }
        ...
        fprintf(stderr,
                "ds4: mtp batch-probe n2 pos=%u label=%s draft0=%d future=%d "
                "seq_ok=%d fast_ok=%d row_top=%d fast0_top=%d fast1_top=%d "
                "seq0_top=%d seq_next_top=%d delta_seq0=%.6g delta_base0=%.6g "
                "prefix_ok=%d prefix_next_top=%d prefix_delta=%.6g row_fallback=%d\n",
                start,
                labels[v],
                drafts[0],
                variants[v],
    ...
    if (getenv("DS4_MTP_BATCH_LAYER_PROBE") != NULL) {
        uint64_t n1_attn[DS4_N_LAYER];
        uint64_t n1_ffn[DS4_N_LAYER];
        uint64_t n2_attn[DS4_N_LAYER];
        uint64_t n2_ffn[DS4_N_LAYER];
        ds4_metal_stage_capture n1_stages;
        ds4_metal_stage_capture n2_stages;
        const bool stage_probe = getenv("DS4_MTP_BATCH_STAGE_PROBE") != NULL;
        uint32_t stage_layer = 0;
        const char *stage_layer_env = getenv("DS4_MTP_BATCH_STAGE_LAYER");
        if (stage_layer_env && stage_layer_env[0]) {
            stage_layer = (uint32_t)strtoul(stage_layer_env, NULL, 10);
        }
        memset(n1_attn, 0, sizeof(n1_attn));
        memset(n1_ffn, 0, sizeof(n1_ffn));
        memset(n2_attn, 0, sizeof(n2_attn));
        memset(n2_ffn, 0, sizeof(n2_ffn));
        memset(&n1_stages, 0, sizeof(n1_stages));
        memset(&n2_stages, 0, sizeof(n2_stages));

        s->checkpoint.len = saved_len;
        ok = spec_frontier_restore(&frontier, s);
        bool n1_layers_ok = ok;
        if (n1_layers_ok) {
            token_vec_push(&s->checkpoint, drafts[0]);
            n1_layers_ok = metal_graph_probe_suffix_layer_hashes(&s->graph,
                                                                 &e->model,
                                                                 &e->weights,
                                                                 &s->checkpoint,
                                                                 start,
                                                                 ...
                                                                 n1_attn,
                                                                 n1_ffn,
                                                                 stage_probe ? &n1_stages : NULL,
                                                                 stage_layer);
        }
        s->checkpoint.len = saved_len;
        ok = spec_frontier_restore(&frontier, s);
        bool n2_layers_ok = ok;
        if (n2_layers_ok) {
            token_vec_push(&s->checkpoint, drafts[0]);
            token_vec_push(&s->checkpoint, variants[0]);
            n2_layers_ok = metal_graph_probe_suffix_layer_hashes(&s->graph,
                                                                 &e->model,
                                                                 &e->weights,
                                                                 &s->checkpoint,
                                                                 start,
        ...
        int first_ffn = -1;
        for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
            if (first_attn < 0 && n1_attn[il] != n2_attn[il]) first_attn = (int)il;
            if (first_ffn < 0 && n1_ffn[il] != n2_ffn[il]) first_ffn = (int)il;
        }
        fprintf(stderr,
                "ds4: mtp batch-probe layers pos=%u future=%d n1_ok=%d n2_ok=%d "
                "first_attn=%d first_ffn=%d "
                "attn_hashes=%016llx/%016llx ffn_hashes=%016llx/%016llx "
                "row_fallback=%d\n",
                start,
                variants[0],
                n1_layers_ok ? 1 : 0,
                ...
                first_attn,
                first_ffn,
                first_attn >= 0 ? (unsigned long long)n1_attn[first_attn] : 0ull,
                first_attn >= 0 ? (unsigned long long)n2_attn[first_attn] : 0ull,
                first_ffn >= 0 ? (unsigned long long)n1_ffn[first_ffn] : 0ull,
                first_ffn >= 0 ? (unsigned long long)n2_ffn[first_ffn] : 0ull,
                getenv("DS4_MTP_VERIFY_ROW_FALLBACK") != NULL ? 1 : 0);

        if (stage_probe) {
            int first_stage = -1;
            const uint32_t stage_count = n1_stages.len < n2_stages.len ? n1_stages.len : n2_stages.len;
            for (uint32_t i = 0; i < stage_count; i++) {
                if (strcmp(n1_stages.items[i].name, n2_stages.items[i].name) != 0 ||
                    n1_stages.items[i].n_f32 != n2_stages.items[i].n_f32 ||
                    n1_stages.items[i].hash != n2_stages.items[i].hash)
                {
                    first_stage = (int)i;
                    break;
                }
            }
            if (first_stage < 0 && n1_stages.len != n2_stages.len) {
                first_stage = (int)stage_count;
            }
            const char *first_name = "none";
            uint64_t first_n1 = 0;
            uint64_t first_n2 = 0;
            uint64_t first_len = 0;
            if (first_stage >= 0 && (uint32_t)first_stage < stage_count) {
                first_name = n1_stages.items[first_stage].name;
                first_n1 = n1_stages.items[first_stage].hash;
                first_n2 = n2_stages.items[first_stage].hash;
                first_len = n1_stages.items[first_stage].n_f32;
            }
            fprintf(stderr,
                    "ds4: mtp batch-probe stages pos=%u layer=%u n1_ok=%d n2_ok=%d "
                    "n1_count=%u n2_count=%u first_stage=%d first_name=%s row_f32=%llu "
                    "hashes=%016llx/%016llx overflow=%d/%d row_fallback=%d\n",
                    start,
                    stage_layer,
                    n1_layers_ok ? 1 : 0,
                    n2_layers_ok ? 1 : 0,
                    ...
                    first_stage,
                    first_name,
                    (unsigned long long)first_len,
                    (unsigned long long)first_n1,
                    (unsigned long long)first_n2,
    ...
    memcpy(s->logits, saved_logits, (size_t)DS4_N_VOCAB * sizeof(s->logits[0]));
    free(prefix_next_logits);
    free(base_fast0_logits);
    free(fast_logits);
    free(n1_logits);
    free(seq_next_logits);
    free(seq0_logits);
    free(saved_logits);
    spec_frontier_free(&frontier);
}

static void ds4_mtp_live_batch_oracle_check(ds4_session *s,
                                            const int *drafts,
                                            int draft_n,
                                            uint32_t start) {
    if (!s || !s->engine || !drafts || draft_n < 2 || s->mtp_oracle_reported) return;
    ds4_engine *e = s->engine;
    ds4_spec_frontier frontier;
    memset(&frontier, 0, sizeof(frontier));
    const int saved_len = s->checkpoint.len;
    bool ok = spec_frontier_snapshot(&frontier, s);
    if (!ok) {
        fprintf(stderr, "ds4: mtp live-batch oracle failed to snapshot state\n");
        s->mtp_oracle_reported = true;
        return;
    }

    float *saved_logits = xmalloc((size_t)DS4_N_VOCAB * sizeof(saved_logits[0]));
    float *seq_logits = xmalloc((size_t)(draft_n + 1) * DS4_N_VOCAB * sizeof(seq_logits[0]));
    float *fast_logits = xmalloc((size_t)draft_n * DS4_N_VOCAB * sizeof(fast_logits[0]));
    int *seq_tops = xmalloc((size_t)(draft_n + 1) * sizeof(seq_tops[0]));
    int *fast_tops = xmalloc((size_t)draft_n * sizeof(fast_tops[0]));
    for (int i = 0; i < draft_n + 1; i++) seq_tops[i] = -1;
    for (int i = 0; i < draft_n; i++) fast_tops[i] = -1;
    memcpy(saved_logits, s->logits, (size_t)DS4_N_VOCAB * sizeof(saved_logits[0]));

    bool seq_ok = true;
    for (int i = 0; i < draft_n && seq_ok; i++) {
        seq_ok = metal_graph_eval_token_raw_swa(&s->graph,
                                                &e->model,
                                                &e->weights,
                                                drafts[i],
                                                start + (uint32_t)i,
                                                seq_logits + (size_t)i * DS4_N_VOCAB);
        if (seq_ok) {
            token_vec_push(&s->checkpoint, drafts[i]);
            seq_tops[i] = sample_argmax(seq_logits + (size_t)i * DS4_N_VOCAB,
                                        DS4_N_VOCAB);
        }
    }
    int seq_state_next_top = -1;
    const int seq_next = seq_ok ? seq_tops[draft_n - 1] : -1;
    if (seq_ok && seq_next >= 0 && start + (uint32_t)draft_n < (uint32_t)s->ctx_size) {
        seq_ok = metal_graph_eval_token_raw_swa(&s->graph,
                                                &e->model,
                                                &e->weights,
                                                seq_next,
                                                start + (uint32_t)draft_n,
                                                seq_logits + (size_t)draft_n * DS4_N_VOCAB);
        if (seq_ok) {
            seq_state_next_top =
                sample_argmax(seq_logits + (size_t)draft_n * DS4_N_VOCAB,
                              DS4_N_VOCAB);
        }
    }
    ...
    bool fast_ok = metal_graph_verify_suffix_tops(&s->graph,
                                                  &e->model,
                                                  &e->weights,
                                                  &s->checkpoint,
                                                  start,
    ...
    int first_row_mismatch = -1;
    int fast_final_top = -1;
    int fast_state_next_top = -1;
    if (fast_ok) {
        fast_final_top =
            sample_argmax(fast_logits + (size_t)(draft_n - 1) * DS4_N_VOCAB,
                          DS4_N_VOCAB);
        for (int i = 0; i + 1 < draft_n; i++) {
            if (fast_tops[i] != seq_tops[i]) {
                first_row_mismatch = i;
                break;
            }
        }
        if (seq_next >= 0 && start + (uint32_t)draft_n < (uint32_t)s->ctx_size) {
            if (metal_graph_eval_token_raw_swa(&s->graph,
                                               ...
                                               start + (uint32_t)draft_n,
                                               s->logits)) {
                fast_state_next_top = sample_argmax(s->logits, DS4_N_VOCAB);
            }
        }
    }

    const bool final_mismatch = fast_final_top != seq_tops[draft_n - 1];
    const bool state_mismatch =
        seq_state_next_top >= 0 &&
        fast_state_next_top >= 0 &&
        fast_state_next_top != seq_state_next_top;
    const bool diverged = !seq_ok || !fast_ok || first_row_mismatch >= 0 ||
                          final_mismatch || state_mismatch;
    if (diverged || getenv("DS4_MTP_ORACLE_LOG_OK") != NULL) {
        const int row = first_row_mismatch >= 0 ? first_row_mismatch : draft_n - 1;
        const float *seq_row =
            row >= 0 && row < draft_n ? seq_logits + (size_t)row * DS4_N_VOCAB : NULL;
        const float *fast_row =
            row >= 0 && row < draft_n ? fast_logits + (size_t)row * DS4_N_VOCAB : NULL;
        const float delta = seq_row && fast_row ?
            max_abs_diff(seq_row, fast_row, DS4_N_VOCAB) : -1.0f;
        fprintf(stderr,
                "ds4: mtp live-batch oracle %s pos=%u draft_n=%d drafts=",
                diverged ? "divergence" : "ok",
                start,
                draft_n);
        ...
        fprintf(stderr,
                " seq_ok=%d fast_ok=%d row_mismatch=%d "
                "seq_row_top=%d fast_row_top=%d final_seq=%d final_fast=%d "
                "state_seq=%d state_fast=%d delta=%.6g\n",
                seq_ok ? 1 : 0,
                fast_ok ? 1 : 0,
                first_row_mismatch,
                row >= 0 && row < draft_n ? seq_tops[row] : -1,
                row >= 0 && row + 1 < draft_n ? fast_tops[row] : fast_final_top,
                seq_tops[draft_n - 1],
                fast_final_top,
                seq_state_next_top,
                fast_state_next_top,
...
int ds4_engine_open(ds4_engine **out, const ds4_engine_options *opt) {
    ...
    if (e->mtp_speed) {
        fprintf(stderr,
                "ds4: warning: --mtp-speed is approximate and may change or degrade output; "
                "use --mtp without --mtp-speed for correctness-preserving generation\n");
    }
...
static int ds4_session_eval_internal(ds4_session *s, int token, bool probe_mtp,
    ...
    if (mtp_should_draft) {
        ...
        if (probe_ok) {
            if (mtp_probe_need_logits) {
                int top0 = -1;
                float v0 = 0.0f, v1 = 0.0f;
                logits_top2(s->mtp_logits, DS4_N_VOCAB, &top0, &v0, NULL, &v1);
                s->mtp_draft_token = mtp_top >= 0 ? mtp_top : top0;
                s->mtp_draft_margin = v0 - v1;
                s->mtp_draft_margin_valid = true;
            } else {
...
int ds4_session_eval_no_mtp_probe(ds4_session *s, int token, char *err, size_t errlen) {
    return ds4_session_eval_internal(s, token, false, err, errlen);
}

#ifndef DS4_NO_METAL
static void ds4_verify_profile_add(ds4_verify_suffix_profile *dst,
                                   const ds4_verify_suffix_profile *src) {
    dst->upload_sec += src->upload_sec;
    dst->layer_sec += src->layer_sec;
    dst->head_sec += src->head_sec;
    dst->top_read_sec += src->top_read_sec;
    dst->logits_read_sec += src->logits_read_sec;
    dst->total_sec += src->total_sec;
}
#endif

int ds4_session_mtp_verify_scale(ds4_session *s, int max_n, int repeats, char *err, size_t errlen) {
#ifdef DS4_NO_METAL
    (void)s; (void)max_n; (void)repeats;
    snprintf(err, errlen, "Metal support is not compiled in");
    return 1;
#else
    if (!s || !s->engine) {
    ...
    if (!s->checkpoint_valid || s->checkpoint.len <= 0) {
        snprintf(err, errlen, "session must be synced before verifier scaling");
        return 1;
    }
    ds4_engine *e = s->engine;
    ds4_metal_graph *g = &s->graph;
    if (!g->spec_logits) {
        snprintf(err, errlen, "--mtp-verify-scale requires --mtp so speculative scratch buffers are allocated");
        return 1;
    }
    if (max_n <= 0) max_n = 8;
    if (max_n > 8) max_n = 8;
    ...
    if (room <= 1) {
        snprintf(err, errlen, "not enough context room for verifier scaling");
        return 1;
    }
    if (max_n > room - 1) max_n = room - 1;
    if (max_n <= 0) {
        snprintf(err, errlen, "not enough context room for verifier scaling");
        return 1;
    }
    ...
    double *seq_cum = NULL;
    int *drafts = NULL;
    float *seq_margin = NULL;
    int *seq_runner = NULL;
    bool have_frontier = false;

#define DS4_VERIFY_SCALE_FAIL(...) do { \
        if (err && errlen) snprintf(err, errlen, __VA_ARGS__); \
        rc = 1; \
        goto done; \
    } while (0)

    const int seq_cap = max_n < room - 1 ? max_n + 1 : max_n;

    saved_logits = xmalloc((size_t)DS4_N_VOCAB * sizeof(saved_logits[0]));
    full_logits = xmalloc((size_t)max_n * DS4_N_VOCAB * sizeof(full_logits[0]));
    seq_cum = xcalloc((size_t)seq_cap + 1u, sizeof(seq_cum[0]));
    drafts = xmalloc((size_t)seq_cap * sizeof(drafts[0]));
    seq_margin = xmalloc((size_t)seq_cap * sizeof(seq_margin[0]));
    seq_runner = xmalloc((size_t)seq_cap * sizeof(seq_runner[0]));
    memcpy(saved_logits, s->logits, (size_t)DS4_N_VOCAB * sizeof(saved_logits[0]));

    have_frontier = spec_frontier_snapshot(&frontier, s);
    if (!have_frontier) DS4_VERIFY_SCALE_FAIL("failed to snapshot verifier baseline");

    for (int i = 0; i < seq_cap; i++) {
        int top0 = -1, top1 = -1;
        float v0 = 0.0f, v1 = 0.0f;
        ...
        seq_margin[i] = v0 - v1;
        const double t0 = now_sec();
        bool ok = metal_graph_eval_token_raw_swa(g,
                                                 &e->model,
                                                 &e->weights,
                                                 drafts[i],
                                                 (uint32_t)(saved_len + i),
                                                 s->logits);
        const double t1 = now_sec();
        if (!ok) DS4_VERIFY_SCALE_FAIL("Metal decode failed while collecting verifier-scale baseline");
        seq_cum[i + 1] = seq_cum[i] + (t1 - t0);
        token_vec_push(&s->checkpoint, drafts[i]);
    }
    ...
    memcpy(s->logits, saved_logits, (size_t)DS4_N_VOCAB * sizeof(saved_logits[0]));
    if (!spec_frontier_restore(&frontier, s)) {
        DS4_VERIFY_SCALE_FAIL("failed to restore verifier baseline");
    }
    ...
    if (seq_cap > max_n) fprintf(stderr, " next=%d", drafts[max_n]);
    fputc('\n', stderr);
    ...
    for (size_t ni = 0; ni < sizeof(ns) / sizeof(ns[0]); ni++) {
        const int n = ns[ni];
        if (n > max_n) continue;
        ds4_verify_suffix_profile sum;
        memset(&sum, 0, sizeof(sum));
        double final_row_sec = 0.0;
        double full_logits_read_sec = 0.0;
        double restore_sec = 0.0;
        double prefix_sec = 0.0;
        double exact2_sec = 0.0;
        int exact2_mismatches = 0;
        int top_mismatches = 0;
        int top_checks = (n > 1 ? n - 1 : 0) * repeats;
        int final_mismatches = 0;
        int final_checks = n < seq_cap ? repeats : 0;
        int state_next_mismatches = 0;
        int state_next_checks = (n > 2 && n + 1 < seq_cap) ? repeats : 0;

        for (int r = 0; r < repeats; r++) {
            int row_tops[16];
            memset(row_tops, 0, sizeof(row_tops));

            s->checkpoint.len = saved_len;
            memcpy(s->logits, saved_logits, (size_t)DS4_N_VOCAB * sizeof(saved_logits[0]));
            const double restore0 = now_sec();
            bool ok = spec_frontier_restore(&frontier, s);
            const double restore1 = now_sec();
            restore_sec += restore1 - restore0;
            if (!ok) DS4_VERIFY_SCALE_FAIL("failed to restore verifier baseline");

            if (n == 2) {
                int exact_top0 = -1;
                const double exact0 = now_sec();
                ok = metal_graph_verify_decode2_exact(g,
                                                       &e->model,
                                                       &e->weights,
                                                       drafts[0],
                                                       drafts[1],
                                                       ...
                                                       &exact_top0,
                                                       full_logits,
                                                       full_logits + DS4_N_VOCAB);
                const double exact1 = now_sec();
                exact2_sec += exact1 - exact0;
                if (!ok) DS4_VERIFY_SCALE_FAIL("exact N=2 verifier failed");
                if (exact_top0 != drafts[1]) exact2_mismatches++;
                s->checkpoint.len = saved_len;
                memcpy(s->logits, saved_logits, (size_t)DS4_N_VOCAB * sizeof(saved_logits[0]));
                if (!spec_frontier_restore(&frontier, s)) {
                    DS4_VERIFY_SCALE_FAIL("failed to restore verifier baseline");
            ...
            for (int i = 0; i < n; i++) token_vec_push(&s->checkpoint, drafts[i]);
            ...
            ok = metal_graph_verify_suffix_tops(g,
                                                ...
                                                NULL,
                                                &prof);
            if (!ok) DS4_VERIFY_SCALE_FAIL("suffix verifier failed at n=%d", n);
            ds4_verify_profile_add(&sum, &prof);

            for (int i = 1; i < n; i++) {
                if (row_tops[i - 1] != drafts[i]) top_mismatches++;
            }

            const double final0 = now_sec();
            ok = metal_graph_read_spec_logits_row(g, (uint32_t)(n - 1), s->logits);
            const double final1 = now_sec();
            final_row_sec += final1 - final0;
            if (!ok) DS4_VERIFY_SCALE_FAIL("failed to read final verifier logits at n=%d", n);
            if (n < seq_cap) {
                const int final_top = sample_argmax(s->logits, DS4_N_VOCAB);
                if (final_top != drafts[n]) {
                    final_mismatches++;
                    if (getenv("DS4_MTP_VERIFY_SCALE_DETAIL") != NULL && r == 0) {
                        int fast_top0 = -1, fast_top1 = -1;
                        float fast_v0 = 0.0f, fast_v1 = 0.0f;
                        logits_top2(s->logits,
                                    DS4_N_VOCAB,
                                    &fast_top0,
                                    &fast_v0,
                                    &fast_top1,
                                    &fast_v1);
                        const int expected = drafts[n];
                        const float candidate_logit =
                            expected >= 0 && expected < (int)DS4_N_VOCAB ?
                            s->logits[expected] : DS4_NEG_INF;
                        fprintf(stderr,
                                "ds4: mtp verify-detail n=%d final_row=%d next_pos=%d "
                                "expected_next=%d fast_top=%d fast_runner=%d exact_runner=%d "
                                "exact_margin=%.6f fast_margin=%.6f "
                                "candidate_gap=%.6f candidate_logit=%.6f fast_top_logit=%.6f\n",
                                n,
                                n - 1,
                                saved_len + n,
                                expected,
            ...
            const double full0 = now_sec();
            ok = ds4_metal_tensor_read(g->spec_logits,
                                       0,
                                       full_logits,
                                       (uint64_t)n * DS4_N_VOCAB * sizeof(full_logits[0])) != 0;
            const double full1 = now_sec();
            full_logits_read_sec += full1 - full0;
            if (!ok) DS4_VERIFY_SCALE_FAIL("failed to read verifier logits at n=%d", n);

            if (getenv("DS4_MTP_VERIFY_SCALE_DETAIL") != NULL && r == 0) {
                for (int i = 1; i < n; i++) {
                    if (row_tops[i - 1] == drafts[i]) continue;
                    const float *row =
                        full_logits + (size_t)(i - 1) * (size_t)DS4_N_VOCAB;
                    int fast_top0 = -1, fast_top1 = -1;
                    float fast_v0 = 0.0f, fast_v1 = 0.0f;
                    logits_top2(row,
                                DS4_N_VOCAB,
                                ...
                                &fast_top1,
                                &fast_v1);
                    const int expected = drafts[i];
                    const float candidate_logit =
                        expected >= 0 && expected < (int)DS4_N_VOCAB ?
                        row[expected] : DS4_NEG_INF;
                    fprintf(stderr,
                            "ds4: mtp verify-detail n=%d row=%d pos=%d "
                            "expected=%d fast_top=%d fast_runner=%d exact_runner=%d "
                            "exact_margin=%.6f fast_margin=%.6f "
                            "candidate_gap=%.6f candidate_logit=%.6f fast_top_logit=%.6f\n",
                            n,
                            i - 1,
            ...
            if (n > 2 && n + 1 < seq_cap) {
                ok = metal_graph_eval_token_raw_swa(g,
                                                    ...
                                                    &e->weights,
                                                    drafts[n],
                                                    (uint32_t)(saved_len + n),
                                                    s->logits);
                if (!ok) DS4_VERIFY_SCALE_FAIL("state-next decode failed at n=%d", n);
                const int state_next_top = sample_argmax(s->logits, DS4_N_VOCAB);
                if (state_next_top != drafts[n + 1]) {
                    state_next_mismatches++;
                    if (getenv("DS4_MTP_VERIFY_SCALE_DETAIL") != NULL && r == 0) {
                        int state_top0 = -1, state_top1 = -1;
                        float state_v0 = 0.0f, state_v1 = 0.0f;
                        logits_top2(s->logits,
                                    DS4_N_VOCAB,
                                    &state_top0,
                                    &state_v0,
                                    &state_top1,
                                    &state_v1);
                        const int expected = drafts[n + 1];
                        const float candidate_logit =
                            expected >= 0 && expected < (int)DS4_N_VOCAB ?
                            s->logits[expected] : DS4_NEG_INF;
                        fprintf(stderr,
                                "ds4: mtp verify-detail n=%d state_next_pos=%d "
                                "fed=%d expected_next=%d state_top=%d state_runner=%d exact_runner=%d "
                                "exact_margin=%.6f state_margin=%.6f "
                                "candidate_gap=%.6f candidate_logit=%.6f state_top_logit=%.6f\n",
                                n,
                                saved_len + n + 1,
                                drafts[n],
                                expected,
            ...
            if (n == 2) {
                const double prefix0 = now_sec();
                ok = spec_frontier_commit_prefix1(s);
                const double prefix1 = now_sec();
                prefix_sec += prefix1 - prefix0;
                if (!ok) DS4_VERIFY_SCALE_FAIL("failed to commit prefix1 state");
            }
        }

        const double inv = 1000.0 / (double)repeats;
        const double seq_ms = seq_cum[n] * 1000.0;
        const double verify_top_ms = sum.total_sec * inv;
        const double final_row_ms = final_row_sec * inv;
        const double verify_plus_final_ms = verify_top_ms + final_row_ms;
        const double budget_1p5_ms = seq_ms / 1.5;
        const double speedup_ceiling =
            verify_plus_final_ms > 0.0 ? seq_ms / verify_plus_final_ms : 0.0;
        fprintf(stderr,
                "ds4: mtp verify-scale n=%d seq=%.3fms budget_1p5=%.3fms "
                "verify_top=%.3fms upload=%.3fms layers=%.3fms head_topk=%.3fms "
                "top_read=%.3fms final_row=%.3fms full_logits_read=%.3fms "
                "prefix1=%.3fms restore=%.3fms speedup_ceiling=%.3fx "
                "top_mismatch=%d/%d final_mismatch=%d/%d state_next_mismatch=%d/%d\n",
                n,
                seq_ms,
                budget_1p5_ms,
                verify_top_ms,
                ...
                sum.layer_sec * inv,
                sum.head_sec * inv,
                sum.top_read_sec * inv,
                final_row_ms,
                full_logits_read_sec * inv,
                prefix_sec * inv,
                restore_sec * inv,
                speedup_ceiling,
                top_mismatches,
                ...
                state_next_checks);
        if (n == 2) {
            const double exact2_ms = exact2_sec * inv;
            fprintf(stderr,
                    "ds4: mtp verify-scale exact2 n=2 seq=%.3fms exact2=%.3fms "
                    "speedup_ceiling=%.3fx top_mismatch=%d/%d\n",
                    seq_ms,
                    exact2_ms,
                    exact2_ms > 0.0 ? seq_ms / exact2_ms : 0.0,
                    exact2_mismatches,
                    repeats);
        }
    }
    ...
    s->checkpoint.len = saved_len;
    if (saved_logits) memcpy(s->logits, saved_logits, (size_t)DS4_N_VOCAB * sizeof(saved_logits[0]));
    if (have_frontier && !spec_frontier_restore(&frontier, s) && rc == 0) {
        if (err && errlen) snprintf(err, errlen, "failed to restore verifier baseline");
        rc = 1;
    }
...
int ds4_session_eval_speculative_argmax(ds4_session *s, int first_token,
    ...
    if (!e->mtp_speed) {
        const int exact_cap = getenv("DS4_MTP_EXACT_DEEP") != NULL ? 4 : 2;
        if (draft_cap > exact_cap) draft_cap = exact_cap;
    }
    ...
    int drafts[16];
    float mtp_margins[16];
    for (int i = 0; i < 16; i++) mtp_margins[i] = -1.0f;
    int draft_n = 1;
     ...
     * only first_token and skip all speculative work.  Speed mode is explicitly
     * approximate and may trust this first MTP token as part of the drafted suffix.
     */
    const bool speed_validate_all = getenv("DS4_MTP_SPEED_VALIDATE") != NULL;
    bool speed_trust_first =
        !strict_mtp &&
        !speed_validate_all &&
        getenv("DS4_MTP_SPEED_VALIDATE_FIRST") == NULL;
    const bool exact_replay_debug = getenv("DS4_MTP_EXACT_REPLAY") != NULL;
    bool speed_trust_suffix =
        !strict_mtp &&
        !exact_replay_debug &&
        !speed_validate_all &&
        getenv("DS4_MTP_SPEED_VALIDATE_SUFFIX") == NULL;
    int speed_validate_prefix = 0;
    const char *speed_validate_prefix_env = getenv("DS4_MTP_SPEED_VALIDATE_PREFIX");
    if (speed_validate_prefix_env && speed_validate_prefix_env[0]) {
        char *end = NULL;
        long v = strtol(speed_validate_prefix_env, &end, 10);
        if (end != speed_validate_prefix_env && v > 0 && v < 16) {
            speed_validate_prefix = (int)v;
        }
    }
    if (!strict_mtp && speed_validate_prefix > 0) {
        speed_trust_first = false;
        speed_trust_suffix = false;
    }
    if (speed_trust_first &&
        mtp_margin_threshold > 0.0f &&
        mtp_margins[0] >= 0.0f &&
        mtp_margins[0] < mtp_margin_threshold)
    {
        speed_trust_first = false;
    }
    const int target_first = sample_argmax(s->logits, DS4_N_VOCAB);
    ...
    if (target_first != drafts[0] && !speed_trust_first) {
    ...
    if (target_first != drafts[0] && getenv("DS4_MTP_SPEC_LOG")) {
        fprintf(stderr,
                "ds4: mtp speed trusting first draft=%d target=%d\n",
                drafts[0],
                target_first);
    }
    ...
    if (!strict_mtp) {
        const char *alt_env = getenv("DS4_MTP_SPEED_ALT_TOPK");
        if (alt_env && alt_env[0]) {
            char *end = NULL;
            long v = strtol(alt_env, &end, 10);
            if (end != alt_env && v > 1) {
                if (v > 16) v = 16;
    ...
    const char *alt_ngram_env = getenv("DS4_MTP_SPEED_ALT_NGRAM");
    if (alt_ngram_env && alt_ngram_env[0]) {
        char *end = NULL;
        long v = strtol(alt_ngram_env, &end, 10);
        if (end != alt_ngram_env && v >= 2 && v <= 8) speed_alt_ngram = (int)v;
    } else {
        const char *repeat_env = getenv("DS4_MTP_SPEED_REPEAT_GUARD");
        if (repeat_env && repeat_env[0]) {
            char *end = NULL;
            long v = strtol(repeat_env, &end, 10);
            if (end != repeat_env && v >= 2 && v <= 8) speed_alt_ngram = (int)v;
        }
    }
    const char *alt_window_env = getenv("DS4_MTP_SPEED_REPEAT_WINDOW");
    if (alt_window_env && alt_window_env[0]) {
        char *end = NULL;
...
#define DS4_MTP_KEEP_ACCEPTED(n_) do { \
    ...
    for (; draft_n < draft_cap; draft_n++) {
        ...
        if (mtp_need_logits) {
            ...
            drafts[draft_n] = mtp_top >= 0 ? mtp_top : top0;
            mtp_margins[draft_n] = v0 - v1;
            mtp_last_top0 = top0;
            mtp_last_top1 = top1;
        ...
        if (speed_alt_topk > 1) {
            int chosen = mtp_top_ids[0] >= 0 ? mtp_top_ids[0] : drafts[draft_n];
            bool all_repeat = true;
            for (int k = 0; k < speed_alt_topk; k++) {
                if (mtp_top_ids[k] < 0) continue;
                if (!ds4_mtp_speed_candidate_repeats(s,
                                                     drafts,
                                                     draft_n,
                                                     mtp_top_ids[k],
                                                     speed_alt_ngram,
                                                     speed_alt_window)) {
                    chosen = mtp_top_ids[k];
                    all_repeat = false;
                    break;
                }
            }
            if (all_repeat && getenv("DS4_MTP_SPEED_ALT_STOP") != NULL) break;
            drafts[draft_n] = chosen;
        }
    ...
    if (mtp_measure) mtp_t_after_draft = now_sec();
    if (draft_n == 2 && getenv("DS4_MTP_ORACLE") != NULL) {
        ds4_mtp_oracle_check_decode2(s, drafts, (uint32_t)s->checkpoint.len);
    }
    if (draft_n >= 2 &&
        (getenv("DS4_MTP_BATCH_FUTURE_PROBE") != NULL ||
         getenv("DS4_MTP_BATCH_N1_N2_PROBE") != NULL ||
         getenv("DS4_MTP_BATCH_PREFIX1_PROBE") != NULL))
    {
        ds4_mtp_batch_probe_check(s, drafts, draft_n, (uint32_t)s->checkpoint.len);
    }
    if (draft_n >= 2 && getenv("DS4_MTP_LIVE_BATCH_ORACLE") != NULL) {
        ds4_mtp_live_batch_oracle_check(s, drafts, draft_n, (uint32_t)s->checkpoint.len);
    }

    if (!strict_mtp && speed_trust_suffix && getenv("DS4_MTP_SPEED_REPEAT_GUARD") != NULL) {
        const int guarded_n = ds4_mtp_speed_repeat_guard_cap(s, drafts, draft_n);
        if (guarded_n < draft_n) {
            if (getenv("DS4_MTP_SPEC_LOG")) {
                fprintf(stderr,
                        "ds4: mtp speed repeat guard capped draft from %d to %d\n",
                        draft_n,
                        guarded_n);
            }
            draft_n = guarded_n;
    ...
    bool speed_margin_reject = false;
    if (speed_trust_suffix && mtp_margin_threshold > 0.0f) {
        for (int i = 0; i < draft_n; i++) {
            if (mtp_margins[i] < mtp_margin_threshold) {
                speed_trust_suffix = false;
                speed_margin_reject = true;
                break;
            }
        }
    }
    ...
    if (!strict_mtp &&
        speed_margin_reject &&
        getenv("DS4_MTP_SPEED_SKIP_LOW_MARGIN") != NULL)
    {
        DS4_MTP_KEEP_ACCEPTED(0);
        const double done = mtp_measure ? now_sec() : 0.0;
        ds4_mtp_stats_record_step(s,
    ...
    if (s->mtp_speed_audit_enabled && !strict_mtp) {
        const int audit_trusted =
            speed_trust_suffix ? draft_n : (speed_trust_first ? 1 : 0);
        if (audit_trusted > 0) {
            ds4_mtp_speed_audit_check(s,
                                      drafts,
                                      mtp_margins,
                                      draft_n,
                                      audit_trusted,
    ...
    if (!strict_mtp && speed_validate_prefix >= 2 && draft_n >= 2) {
        float *row_logits = xmalloc((size_t)DS4_N_VOCAB * sizeof(row_logits[0]));
        int row_tops[16];
        for (int i = 0; i < 16; i++) row_tops[i] = -1;
        uint32_t validate_tokens = (uint32_t)speed_validate_prefix;
        if (validate_tokens > (uint32_t)draft_n) validate_tokens = (uint32_t)draft_n;
        uint32_t validate_rows = validate_tokens > 1u ? validate_tokens - 1u : 1u;
        const int start = s->checkpoint.len;
        for (int i = 0; i < draft_n; i++) token_vec_push(&s->checkpoint, drafts[i]);
        const double verify_t0 = mtp_measure ? now_sec() : 0.0;
        bool ok = metal_graph_verify_suffix_prefix_final(&s->graph,
                                                         &e->model,
                                                         &e->weights,
                                                         &s->checkpoint,
                                                         (uint32_t)start,
        ...
        if (!ok) {
            free(row_logits);
            snprintf(err, errlen, "MTP prefix verifier failed");
            s->checkpoint_valid = false;
            DS4_MTP_KEEP_ACCEPTED(0);
            return -1;
        }
        ...
        int prefix_miss = -1;
        for (uint32_t i = 0; i < validate_rows; i++) {
            if (row_tops[i] != drafts[i + 1u]) {
                prefix_miss = (int)i;
                break;
            }
        }
        ...
        if (prefix_miss < 0) {
            ...
            if (mtp_timing) {
                fprintf(stderr,
                        "ds4: mtp timing speed-prefix drafted=%d committed=%d draft=%.3f ms verify=%.3f ms total=%.3f ms\n",
                        draft_n,
                        draft_n,
                        (mtp_t_after_draft - mtp_t0) * 1000.0,
        ...
        if (!ok) {
            free(row_logits);
            snprintf(err, errlen, "MTP prefix commit failed");
            s->checkpoint_valid = false;
            DS4_MTP_KEEP_ACCEPTED(0);
            return -1;
        }
        ...
        if (mtp_timing) {
            fprintf(stderr,
                    "ds4: mtp timing speed-prefix drafted=%d committed=1 miss_row=%d row_top=%d draft_next=%d draft=%.3f ms verify=%.3f ms prefix=%.3f ms total=%.3f ms\n",
                    draft_n,
                    prefix_miss,
                    prefix_miss >= 0 ? row_tops[prefix_miss] : -1,
                    prefix_miss >= 0 ? drafts[prefix_miss + 1] : -1,
                    (mtp_t_after_draft - mtp_t0) * 1000.0,
                    (verify_done - verify_t0) * 1000.0,
    ...
    if (use_decode_n_exact) {
        ...
        bool have_frontier = spec_frontier_snapshot(&frontier, s);
        const double snapshot_done = mtp_measure ? now_sec() : 0.0;
        bool ok = have_frontier;
        if (ok) {
            ok = metal_graph_verify_decode_exact_n(&s->graph,
                                                   &e->model,
                                                   &e->weights,
                                                   drafts,
                                                   (uint32_t)draft_n,
        ...
        if (ok) {
            ...
            if (commit_drafts == draft_n) {
                ...
                if (mtp_timing) {
                    fprintf(stderr,
                            "ds4: mtp timing exactn drafted=%d committed=%d draft=%.3f ms snapshot=%.3f ms verify=%.3f ms total=%.3f ms\n",
                            draft_n,
                            draft_n,
                            (mtp_t_after_draft - mtp_t0) * 1000.0,
            ...
            if (commit_drafts == 1) {
                ok = spec_frontier_commit_prefix1(s);
                if (ok) {
                    memcpy(s->logits, prefix1_logits, (size_t)DS4_N_VOCAB * sizeof(s->logits[0]));
                    token_vec_push(&s->checkpoint, drafts[0]);
                    accepted[n_accept++] = drafts[0];
                    s->checkpoint_valid = true;
                    s->mtp_draft_valid = false;
                    ...
                    ds4_mtp_stats_record_step(s,
                                              ...
                                              mtp_t_after_draft - mtp_t0,
                                              verify_done - snapshot_done,
                                              (snapshot_done - snapshot_t0) + (prefix_done - verify_done),
                                              0.0,
                                              prefix_done - mtp_t0);
                    spec_frontier_free(&frontier);
                    free(prefix1_logits);
        ...
        free(row_logits);
        if (getenv("DS4_MTP_SPEC_LOG")) {
            fprintf(stderr, "ds4: mtp exactn verifier failed, falling back to sequential\n");
        }
    }
    ...
    if (!use_decode2_exact)
    {
        ...
        const int start = s->checkpoint.len;
        /*
         * Strict mode keeps a rollback point and only commits the verifier-matched
         * prefix.  Explicit speed mode is an approximate generation mode: unless
         * DS4_MTP_SPEED_VALIDATE is set, it trusts the drafted suffix after the
         * batch verifier has built continuation state for those tokens.
         */
        ...
        const double micro_verify_done = mtp_measure ? now_sec() : 0.0;
        if (ok) {
            int commit_drafts = 1;
            int verifier_mismatches = 0;
            if (speed_trust_suffix) {
            ...
            if (mtp_conf_log) {
                fprintf(stderr,
                        "ds4: mtp conf drafted=%d committed=%d mtp_top=%d runner=%d margin=%.6f target_next=%d draft_next=%d trust=%d mismatches=%d\n",
                        draft_n,
                        commit_drafts,
                        mtp_last_top0,
                        mtp_last_top1,
                        mtp_last_margin,
                        (!speed_trust_suffix && draft_n > 1) ? row_tops[0] : -1,
                ...
                if (ok) {
                    ...
                    if (ok) {
                        ...
                        ds4_mtp_stats_record_step(s,
                                                  ...
                                                  replayed,
                                                  mtp_t_after_draft - mtp_t0,
                                                  micro_verify_done - snapshot_done,
                                                  snapshot_done - snapshot_t0,
                                                  replay_done - micro_verify_done,
                                                  replay_done - mtp_t0);
                        spec_frontier_free(&frontier);
                        free(row_logits);
                        free(row_tops);
            ...
            if (commit_drafts == draft_n) {
                ...
                if (ok) {
                    ...
                    if (mtp_timing) {
                        fprintf(stderr,
                                "ds4: mtp timing micro drafted=%d committed=%d draft=%.3f ms snapshot=%.3f ms verify=%.3f ms total=%.3f ms trust=%d mismatches=%d\n",
                                draft_n,
                                draft_n,
                                (mtp_t_after_draft - mtp_t0) * 1000.0,
                                (snapshot_done - snapshot_t0) * 1000.0,
            ...
            if (commit_drafts == 1 && capture_prefix1) {
                s->checkpoint.len = start;
                const double prefix_t0 = mtp_measure ? now_sec() : 0.0;
                ok = spec_frontier_commit_prefix1(s);
                const double prefix_done = mtp_measure ? now_sec() : 0.0;
            ...
            if (ok && strict_mtp && commit_drafts == 2) {
                int row0_top = -1;
                float *row0_logits = xmalloc((size_t)DS4_N_VOCAB * sizeof(row0_logits[0]));
                ok = metal_graph_verify_decode2_exact(&s->graph,
                                                      &e->model,
                                                      ...
                                                      row0_logits,
                                                      row_logits);
                if (ok && row0_top != drafts[1]) ok = false;
                if (ok) {
                    memcpy(s->logits, row_logits, (size_t)DS4_N_VOCAB * sizeof(s->logits[0]));
                    token_vec_push(&s->checkpoint, drafts[0]);
                    token_vec_push(&s->checkpoint, drafts[1]);
                    ...
                    if (mtp_timing) {
                        fprintf(stderr,
                                "ds4: mtp timing micro drafted=%d committed=%d draft=%.3f ms snapshot=%.3f ms verify=%.3f ms exact2_replay=%.3f ms total=%.3f ms\n",
                                draft_n,
                                commit_drafts,
                                (mtp_t_after_draft - mtp_t0) * 1000.0,
                                (snapshot_done - snapshot_t0) * 1000.0,
    ...
    const double seq_t0 = mtp_measure ? now_sec() : 0.0;
    for (int i = 0; i < draft_n && n_accept < accepted_cap; i++) {
...
#undef DS4_MTP_KEEP_ACCEPTED
    const double seq_done = mtp_measure ? now_sec() : 0.0;
    ds4_mtp_stats_record_step(s,
                              DS4_MTP_STATS_SEQ,
                              draft_n,
                              verified,
    ...
    if (mtp_timing) {
        fprintf(stderr,
                ...
                verified,
                (mtp_t_after_draft - mtp_t0) * 1000.0,
                (seq_done - seq_t0) * 1000.0,
                (seq_done - mtp_t0) * 1000.0);
    }
    if (getenv("DS4_MTP_SPEC_LOG")) {
        if (verified == draft_n) {
            fprintf(stderr,
