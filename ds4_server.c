 ...
 * blocking thread that parses one request, then queues a job to the Metal
 * worker.  The worker owns either the serialized ds4_session or the multi-slot
 * ds4_batch runtime and therefore owns all live KV cache state.  That keeps
 * session reuse, disk checkpointing, and batching decisions in one place
 * instead of spreading graph mutations across client threads. */
...
static char *responses_special_schema_from_tool(const char *raw) {
    ...
    if (type && !strcmp(type, "tool_search")) {
        ...
        json_escape(&b, description ? description : "Search available app and connector tools.");
        buf_puts(&b, ",\"parameters\":");
        buf_puts(&b, parameters ? parameters : "{\"type\":\"object\",\"properties\":{}}");
        buf_putc(&b, '}');
        out = buf_take(&b);
    }
...
static void tool_schema_orders_add_json_wire(tool_schema_orders *orders, const char *json,
                                             const char *namespace, const char *wire_name) {
...
static bool append_responses_namespace_tool_schemas(buf *schemas,
    ...
    while (*tp && *tp != ']') {
        ...
        char *wire_name = NULL;
        char *schema = responses_namespace_function_schema_from_tool(tool_raw, name, &wire_name);
        if (schema) {
...
static bool parse_responses_input_item(const char **p, chat_msgs *msgs,
    ...
    {
        if (!strcmp(type, "tool_search_output") && output &&
            loaded_tool_schemas && orders)
        {
            const char *tools_p = output;
            char *schemas = NULL;
...
static bool parse_responses_input(const char **p, chat_msgs *msgs,
    ...
    json_ws(p);
    while (**p && **p != ']') {
        if (!parse_responses_input_item(p, msgs, loaded_tool_schemas, orders)) return false;
        json_ws(p);
        if (**p == ',') (*p)++;
        json_ws(p);
    }
...
static bool parse_responses_request(ds4_engine *e, server *s, const char *body, int def_tokens,
    ...
    if (loaded_tool_schemas.ptr && loaded_tool_schemas.ptr[0]) {
        if (combined_tool_schemas.len) buf_putc(&combined_tool_schemas, '\n');
        buf_puts(&combined_tool_schemas, loaded_tool_schemas.ptr);
    }
    ...
    kv_cache_restore_tool_memory_for_messages(s, &msgs);
    tool_memory_attach_to_messages(s, &msgs, &r->tool_replay);
    r->prompt_text = render_chat_prompt_text(&msgs, r->has_tools ? effective_tool_schemas : NULL,
                                             &r->tool_orders, r->think_mode);
    ds4_tokenize_rendered_chat(e, r->prompt_text, &r->prompt);
...
static void append_responses_tool_search_call_item(buf *b, const tool_call *tc,
                                                   const char *id, int i,
                                                   const tool_schema_orders *orders) {
    ...
    buf_puts(b, "{\"id\":");
    json_escape(b, item_id);
    buf_puts(b, ",\"type\":\"tool_search_call\",\"call_id\":");
    json_escape(b, tc->id && tc->id[0] ? tc->id : call_id);
    buf_puts(b, ",\"status\":\"completed\",\"execution\":\"client\",\"arguments\":");
...
static void append_responses_function_call_item(buf *b, const tool_call *tc, const char *id,
    ...
    if (responses_tool_name_is_tool_search(tc->name)) {
        append_responses_tool_search_call_item(b, tc, id, i, orders);
        return;
    }
    ...
    json_escape(b, tc->id && tc->id[0] ? tc->id : call_id);
    buf_puts(b, ",\"name\":");
    json_escape(b, order && order->wire_name ? order->wire_name : (tc->name ? tc->name : ""));
...
static const char *responses_stream_call_id(server *s, responses_stream *st, int index) {
    ...
    if (index >= st->call_ids_cap) {
        ...
        int cap = old ? old : 4;
        while (cap <= index) cap *= 2;
        st->call_ids = xrealloc(st->call_ids, (size_t)cap * sizeof(st->call_ids[0]));
        memset(st->call_ids + old, 0, (size_t)(cap - old) * sizeof(st->call_ids[0]));
        st->call_ids_cap = cap;
    }
...
static void responses_stream_free(responses_stream *st) {
    ...
    for (int i = 0; i < st->call_ids_cap; i++) free(st->call_ids[i]);
    free(st->call_ids);
    st->call_ids = NULL;
    st->call_ids_cap = 0;
}
...
static bool responses_sse_function_call_added(int fd, server *s, const char *id,
    ...
    buf_puts(&b, ",\"name\":");
    json_escape(&b, order && order->wire_name ? order->wire_name : (ts->name ? ts->name : ""));
    if (order && order->namespace) {
        buf_puts(&b, ",\"namespace\":");
        json_escape(&b, order->namespace);
    }
...
static bool responses_tool_emit_args_fragment(int fd, const char *id,
    ...
    if (responses_tool_name_is_tool_search(ts->name)) return true;
...
static bool responses_sse_tool_search_item_done_live(int fd, server *s, const char *id,
    ...
    json_escape(&b, id);
    buf_printf(&b, ",\"output_index\":%d,\"item\":{\"type\":\"tool_search_call\",\"id\":",
               ts->output_index);
    json_escape(&b, item_id);
    buf_puts(&b, ",\"call_id\":");
    json_escape(&b, call_id);
    buf_puts(&b, ",\"status\":\"completed\",\"execution\":\"client\",\"arguments\":");
    append_json_object_ordered_or_empty(&b, ts->args.ptr ? ts->args.ptr : "{}", order);
    buf_puts(&b, "}}");
...
static int server_ctx(const server *s) {
    if (!s) return 0;
    if (s->batch) return ds4_batch_ctx(s->batch);
...
static void apply_responses_stream_tool_ids(tool_calls *calls,
    ...
    if (!calls || !st) return;
    int n = calls->len < st->call_ids_cap ? calls->len : st->call_ids_cap;
    for (int i = 0; i < n; i++) {
        if (calls->v[i].id && calls->v[i].id[0]) continue;
        if (st->call_ids[i] && st->call_ids[i][0]) calls->v[i].id = xstrdup(st->call_ids[i]);
    }
}
...
static kv_live_state kv_live_session(ds4_session *session) {
    return (kv_live_state){ .session = session, .slot = -1 };
}

static kv_live_state kv_live_batch_slot(ds4_batch *batch, int slot) {
    return (kv_live_state){ .batch = batch, .slot = slot };
}

static int kv_live_ctx(kv_live_state live) {
    return live.batch ? ds4_batch_ctx(live.batch) : ds4_session_ctx(live.session);
}

static const ds4_tokens *kv_live_tokens(kv_live_state live) {
    return live.batch ? ds4_batch_tokens(live.batch, live.slot) : ds4_session_tokens(live.session);
}

static uint64_t kv_live_payload_bytes(kv_live_state live) {
    return live.batch ?
        ds4_batch_slot_payload_bytes(live.batch, live.slot) :
        ds4_session_payload_bytes(live.session);
}

static bool kv_live_can_save(kv_live_state live) {
    return live.batch ? ds4_batch_slot_can_save(live.batch, live.slot) : live.session != NULL;
}

static uint64_t kv_live_generation(kv_live_state live) {
    return live.batch ? ds4_batch_slot_generation(live.batch, live.slot) : 0;
}

static int kv_live_save_payload(kv_live_state live, FILE *fp, char *err, size_t errlen) {
    return live.batch ?
        ds4_batch_save_slot_payload(live.batch, live.slot, fp, err, errlen) :
        ds4_session_save_payload(live.session, fp, err, errlen);
}

static int kv_live_load_payload(kv_live_state live, FILE *fp, uint64_t payload_bytes,
                                char *err, size_t errlen) {
    return live.batch ?
        ds4_batch_load_slot_payload(live.batch, live.slot, fp, payload_bytes, err, errlen) :
        ds4_session_load_payload(live.session, fp, payload_bytes, err, errlen);
}

static void kv_live_invalidate(kv_live_state live) {
    if (live.batch) ds4_batch_invalidate_slot(live.batch, live.slot);
    else ds4_session_invalidate(live.session);
}

static bool kv_cache_store_state_prefix(server *s, kv_live_state live,
    ...
    char err[160] = {0};
    if (!kv_live_can_save(live)) {
        server_log(DS4_LOG_KVCACHE,
                   "ds4-server: kv cache skipped tokens=%d reason=%s because live checkpoint is not snapshot-safe",
                   store_tokens.len,
                   reason);
        ds4_tokens_free(&store_tokens);
        return false;
    ...
    const ds4_tokens *live_tokens = kv_live_tokens(live);
    if (!live_tokens ||
    ...
    uint64_t payload_bytes = kv_live_payload_bytes(live);
    if (payload_bytes == 0) {
        ds4_tokens_free(&store_tokens);
        return false;
    }
    ...
    buf tmpb = {0};
    buf_printf(&tmpb, "%s.tmp.%ld.%d.%llu",
               path,
               (long)getpid(),
               live.batch ? live.slot : -1,
               (unsigned long long)kv_live_generation(live));
    ...
    kv_fill_header(h, (uint8_t)quant_bits, kv_reason_code(reason), 0,
                   ...
                   (uint32_t)kv_live_ctx(live), now, now, payload_bytes);
    uint8_t tb[4];
    ...
    bool ok = fwrite(h, 1, sizeof(h), fp) == sizeof(h) &&
              ...
              kv_live_save_payload(live, fp, err, sizeof(err)) == 0 &&
...
static bool kv_cache_store_live_prefix(server *s, const ds4_tokens *tokens,
                                       int store_len, const char *reason) {
    return kv_cache_store_state_prefix(s, kv_live_session(s->session),
                                       tokens, store_len, reason);
}

static bool kv_cache_store_batch_slot_prefix(server *s, int slot, const ds4_tokens *tokens,
                                             int store_len, const char *reason) {
    return kv_cache_store_state_prefix(s, kv_live_batch_slot(s->batch, slot),
                                       tokens, store_len, reason);
}
...
static int kv_cache_try_load_text_state(server *s, kv_live_state live,
    ...
    char err[160] = {0};
    int loaded = 0;
    if (header_ok && kv_live_load_payload(live, fp, hdr.payload_bytes, err, sizeof(err)) == 0) {
        const ds4_tokens *loaded_tokens = kv_live_tokens(live);
...
static int kv_cache_try_load_text(server *s, const char *prompt_text,
                                  ds4_tokens *effective_prompt,
                                  char **loaded_path_out) {
    return kv_cache_try_load_text_state(s, kv_live_session(s->session),
...
static int kv_cache_try_load_batch_slot(server *s, int slot, const request *req,
                                        ds4_tokens *effective_prompt,
                                        char **loaded_path_out) {
    return kv_cache_try_load_text_state(s, kv_live_batch_slot(s->batch, slot),
                                        req ? req->prompt_text : NULL,
                                        effective_prompt, loaded_path_out);
}
...
static void server_progress_cb(void *ud, const char *event, int current, int total) {
    ...
    if (p->seen && current == p->last_current) {
        if (p->srv && p->batch_slot < 0 && current > p->cached_tokens) kv_cache_maybe_store_continued(p->srv);
        return;
    }
...
static void generation_state_begin(server *s, generation_state *g, job *j, int slot,
    ...
    g->srv = s;
    g->j = j;
    ds4_tokens_copy(&g->prompt, prompt ? prompt : &j->req.prompt);
    g->slot = slot;
    g->slot_generation = ds4_batch_slot_generation(s->batch, slot);
    g->cached = cached;
    g->prefill_pos = cached;
    g->continued_last_store_tokens = cached;
    g->finish = "length";
    g->t0 = now_sec();
    g->err[0] = '\0';
    request_ctx_span(g->ctx_span, sizeof(g->ctx_span), cached, g->prompt.len);
    log_flags(g->req_flags, sizeof(g->req_flags), j->req.has_tools, false, false, false);

    trace_cache_diag cache_diag = {0};
    if (s->trace) {
        const ds4_tokens *live = ds4_batch_tokens(s->batch, slot);
        trace_cache_capture(&cache_diag, live, &g->prompt,
                            live ? live->len : 0, cached);
    }
    g->trace_id = trace_begin(s, j, cached, g->prompt.len, &cache_diag,
                              cache_source ? cache_source : (cached > 0 ? "slot" : "none"),
                              disk_cached, disk_path);
    if (cached == 0 &&
        s->kv.enabled &&
        g->prompt.len >= s->kv.opt.min_tokens &&
        s->kv.opt.cold_max_tokens > 0 &&
        g->prompt.len <= s->kv.opt.cold_max_tokens)
    {
        g->cold_store_len = kv_cache_store_len(&s->kv, g->prompt.len);
    }
    server_log(DS4_LOG_PREFILL,
               "ds4-server: batch slot=%d %s ctx=%s%s%s prompt start",
               slot,
               j->req.kind == REQ_CHAT ? "chat" : "completion",
               g->ctx_span,
               g->req_flags[0] ? " " : "",
...
static bool generation_state_slot_current(const generation_state *g) {
    if (!g || !g->srv || !g->srv->batch || g->slot < 0) return false;
    return ds4_batch_slot_occupied(g->srv->batch, g->slot) &&
...
static bool generation_state_start_decode(generation_state *g) {
    ...
    server_log(DS4_LOG_PREFILL,
               "ds4-server: batch slot=%d %s ctx=%s%s%s prompt done %.3fs",
               g->slot,
               j->req.kind == REQ_CHAT ? "chat" : "completion",
               g->ctx_span,
               g->req_flags[0] ? " " : "",
    ...
    g->greedy_top_path = !j->req.has_tools &&
...
static void batch_kv_cache_note_store(generation_state *g, int tokens) {
    if (tokens > g->continued_last_store_tokens) g->continued_last_store_tokens = tokens;
}
...
static void kv_cache_maybe_store_batch_continued(server *s, generation_state *g) {
    ...
    if (tokens->len - g->continued_last_store_tokens < kc->opt.continued_interval_tokens) return;
    if (kv_cache_store_batch_slot_prefix(s, g->slot, tokens, tokens->len, "continued")) {
        batch_kv_cache_note_store(g, tokens->len);
    }
}
...
static bool generation_state_accept_token(generation_state *g, int token) {
    ...
    if (j->req.stream && j->req.api == API_ANTHROPIC &&
        !anthropic_sse_stream_update(j->fd, &j->req, g->id,
    ...
    if (j->req.stream && j->req.api == API_RESPONSES &&
        !responses_sse_stream_update(j->fd, s, &j->req, g->id,
    ...
    if (j->req.kind == REQ_CHAT && j->req.has_tools) {
        ...
        if (orphan_end && !g->saw_orphan_tool_end) {
            ...
            server_log(DS4_LOG_WARNING,
                       "ds4-server: batch slot=%d chat ctx=%s ignored orphan tool-call end marker after %d generated tokens",
                       g->slot,
                       g->ctx_span,
                       g->completion);
            trace_event(s, g->trace_id,
        ...
        const size_t marker_hold = 80;
        g->tool_scan_from = g->text.len > marker_hold ? g->text.len - marker_hold : 0;
...
static void generation_state_finish(generation_state *g) {
    ...
    if (j->req.stream) {
        bool response_ok = true;
        if (j->req.api == API_RESPONSES) {
            response_ok = responses_sse_finish_live(j->fd, s, &j->req, g->id, &g->responses_live,
                                                    g->text.ptr ? g->text.ptr : "", g->text.len,
                                                    parsed_content ? parsed_content : "",
                                                    &parsed_calls,
                                                    g->prompt.len, g->completion);
        } else if (j->req.api == API_ANTHROPIC) {
            response_ok = anthropic_sse_finish_live(j->fd, &j->req, g->id, &g->anthropic_live,
                                                    g->text.ptr ? g->text.ptr : "", g->text.len,
            ...
            response_ok = sse_chunk(j->fd, &j->req, g->id, NULL, final_finish) &&
                          sse_done(j->fd, &j->req, g->id, g->prompt.len, g->completion);
        }
        if (!response_ok) {
            final_finish = "error";
            g->finish = "error";
            server_log(DS4_LOG_DEFAULT,
                       "ds4-server: batch slot=%d %s ctx=%s final stream failed",
                       g->slot,
                       j->req.kind == REQ_CHAT ? "chat" : "completion",
                       g->ctx_span);
        }
    ...
    if (!strcmp(final_finish, "error") && g->err[0]) {
        server_log(DS4_LOG_GENERATION,
                   "ds4-server: batch slot=%d %s ctx=%s gen=%d%s%s finish=%s error=\"%s\" %.3fs",
                   g->slot,
                   j->req.kind == REQ_CHAT ? "chat" : "completion",
                   g->ctx_span,
                   g->completion,
        ...
        server_log(DS4_LOG_GENERATION,
                   "ds4-server: batch slot=%d %s ctx=%s gen=%d%s%s finish=%s %.3fs",
                   g->slot,
                   j->req.kind == REQ_CHAT ? "chat" : "completion",
                   g->ctx_span,
                   g->completion,
...
static job *try_dequeue(server *s) {
    pthread_mutex_lock(&s->mu);
    job *j = s->head;
    if (j) {
        s->head = j->next;
...
static bool batch_has_short_prefill(const batch_request *reqs, int n, int chunk_tokens) {
    for (int i = 0; i < n; i++) {
        ...
        const int remaining = g->prompt.len - g->prefill_pos;
...
static bool batch_live_prefix_matches(server *s, const generation_state *g, int prefix_len) {
    ...
    const ds4_tokens *prompt = &g->prompt;
...
static bool batch_prefill_step_wants_logits(server *s, generation_state *g, int next_pos) {
    if (next_pos >= g->prompt.len) return true;
...
static int batch_prefill_align_tokens(void) {
    const char *env = getenv("DS4_BATCH_PREFILL_ALIGN_TOKENS");
    if (env && env[0]) {
        char *end = NULL;
        long v = strtol(env, &end, 10);
        if (end != env && *end == '\0' && v >= 0 && v <= INT_MAX) return (int)v;
...
static int batch_prefill_segment_end(server *s, generation_state *g, int max_tokens) {
    const int start = g->prefill_pos;
    int end = start + max_tokens;
    if (end > g->prompt.len) end = g->prompt.len;
    const int align = batch_prefill_align_tokens();
    if (align > 1 && end < g->prompt.len) {
        const int rem = start % align;
        if (rem != 0) {
            const int boundary = start + (align - rem);
            if (boundary > start && boundary < end) end = boundary;
        } else {
            const int aligned_end = end - (end % align);
            if (aligned_end > start && aligned_end < end) end = aligned_end;
        }
    }
    for (int next = start + 1; next <= end; next++) {
        if (batch_prefill_step_wants_logits(s, g, next)) return next;
    }
    return end;
}
...
static bool batch_prefill_shared_prefix_many(server *s, batch_request *reqs) {
    ...
    for (int i = 0; i < s->max_slots; i++) {
        ...
        if (current >= base->prompt.len) continue;
        int n = 0;
        int common = base->prompt.len;
        bool all_equal = true;
        group[n++] = i;
        for (int j = i + 1; j < s->max_slots; j++) {
            ...
            if (other->prefill_pos != current ||
                other->prefill_pos >= other->prompt.len) continue;
            const int pair_common = batch_prompt_common_prefix(&base->prompt,
                                                               &other->prompt);
    ...
    for (int i = 0; i < best_n; i++) {
        ...
        if (g->cold_store_len == g->prefill_pos &&
            ...
            if (kv_cache_store_batch_slot_prefix(s, g->slot, &g->prompt,
                                                 g->prefill_pos, "cold")) {
...
static bool batch_admit_job(server *s, batch_request *reqs, job *j) {
    ...
    char *disk_path = NULL;
    ds4_tokens effective_prompt = {0};
    const ds4_tokens *prompt_for_slot = &j->req.prompt;
    if (s->kv.enabled) {
        const int quant_bits = ds4_engine_routed_quant_bits(s->engine);
        int disk_candidate = 0;
        if ((quant_bits == 2 || quant_bits == 4) && j->req.prompt_text) {
            int idx = kv_cache_find_text_prefix(&s->kv, j->req.prompt_text, quant_bits, ds4_batch_ctx(s->batch));
        ...
        if (disk_candidate > cached) {
            ...
            if (old_tokens && old_tokens->len >= s->kv.opt.min_tokens &&
                ...
                kv_cache_store_batch_slot_prefix(s, slot, old_tokens, old_tokens->len, "evict");
            }
            disk_cached = kv_cache_try_load_batch_slot(s, slot, &j->req, &effective_prompt, &disk_path);
            if (disk_cached > 0) {
                cached = disk_cached;
                cache_source = "disk-text";
                prompt_for_slot = &effective_prompt;
            } else {
                cached = 0;
                cache_source = "none";
            }
    ...
    generation_state_begin(s, &reqs[slot].gen, j, slot, prompt_for_slot, cached,
                           cache_source, disk_cached, disk_path);
    free(disk_path);
    const int prompt_len = prompt_for_slot->len;
    ds4_tokens_free(&effective_prompt);
    if (cached >= prompt_len) {
        if (generation_state_start_decode(&reqs[slot].gen)) {
            reqs[slot].phase = BATCH_REQ_DECODE;
        } else {
            reqs[slot].phase = BATCH_REQ_DONE;
        }
    }
    return true;
}
...
static bool batch_prefill_many(server *s, batch_request *reqs) {
    ...
    if (n_candidates < 2 || row_budget < 2) return changed;
    ...
    for (int ci = 0; ci < n_candidates && remaining_rows > 0; ci++) {
        ...
        generation_state *g = &reqs[candidates[ci]].gen;
        const int remaining_prompt = g->prompt.len - g->prefill_pos;
        if (remaining_prompt <= 0) continue;
        if (share > remaining_prompt) share = remaining_prompt;
        const int next_pos = batch_prefill_segment_end(s, g, share);
        const int len = next_pos - g->prefill_pos;
        if (len <= 0) continue;
        const bool wants_logits = batch_prefill_step_wants_logits(s, g, next_pos);
        segments[n] = (ds4_batch_prefill_segment){
            .slot = g->slot,
            .tokens = &g->prompt.v[g->prefill_pos],
            .n_tokens = len,
            .refresh_logits = wants_logits ? 1 : 0,
        };
        gens[n] = g;
        seg_lens[n] = len;
        if (wants_logits) n_logits++;
        rows += len;
        remaining_rows -= len;
        n++;
    }
...
static bool batch_prefill_one(server *s, batch_request *reqs) {
    for (int i = 0; i < s->max_slots; i++) {
        ...
        int next = g->prompt.len;
        if (batch_decode_ready_count(reqs, s->max_slots) > 0) {
            next = g->prefill_pos + batch_prefill_chunk_tokens();
            if (next < g->cached) next = g->cached;
            if (next > g->prompt.len) next = g->prompt.len;
        }
        if (next <= 0) next = g->prompt.len;
        ...
        if (g->cold_store_len == next && next >= s->kv.opt.min_tokens) {
            if (kv_cache_store_batch_slot_prefix(s, g->slot, &g->prompt, next, "cold")) {
                batch_kv_cache_note_store(g, next);
            }
            g->cold_store_len = 0;
        }
        kv_cache_maybe_store_batch_continued(s, g);
...
static bool batch_decode_step(server *s, batch_request *reqs) {
    ...
    for (int i = 0; i < s->max_slots; i++) {
        ...
        if (g->greedy_top_path && g->next_token_valid) {
            ...
            token = ds4_batch_sample(s->batch, g->slot, temperature, top_k, top_p, min_p, &g->rng);
            if (token < 0) {
                g->finish = "error";
    ...
    if (n == 0) return false;
    if (s->batch_wait_us > 0 && n < s->max_slots) usleep((useconds_t)s->batch_wait_us);
    char err[160];
    err[0] = '\0';
    int active = 0;
    for (int i = 0; i < s->max_slots; i++) {
...
static void *batch_worker_main(void *arg) {
    server *s = arg;
    batch_request *reqs = xcalloc((size_t)s->max_slots, sizeof(reqs[0]));
    for (;;) {
...
static void append_model_json(buf *b, const server *s) {
    append_model_json_values(b, server_ctx(s), s->default_tokens);
}
...
static void *client_main(void *arg) {
    ...
    char err[160];
    bool ok = false;
    const int ctx_size = server_ctx(s);
...
static void usage(FILE *fp) {
    fprintf(fp,
        ...
        "  --max-slots N\n"
        "      Active inference slots. 1 preserves serialized session mode. >1 enables the multi-slot scheduler. Default: 1\n"
        "  --batch-wait-us N\n"
        ...
        "      In shared-decode batch mode, pack active prompt segments into shared prefill row batches.\n"
        "\n"
        "HTTP API:\n"
        "  --host HOST\n"
...
static server_config parse_options(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (!strcmp(arg, "-h") || !strcmp(arg, "--help")) {
            ...
            if (c.max_slots < 1 || c.max_slots > 64) {
                server_log(DS4_LOG_DEFAULT, "ds4-server: --max-slots must be between 1 and 64");
                exit(2);
            }
            ...
            if (c.batch_wait_us > 100000) {
                server_log(DS4_LOG_DEFAULT, "ds4-server: --batch-wait-us must be between 0 and 100000");
                exit(2);
            }
        } else if (!strcmp(arg, "--batch-backend")) {
            c.batch_backend = need_arg(&i, argc, argv, arg);
            if (strcmp(c.batch_backend, "session-slots") &&
                strcmp(c.batch_backend, "shared-decode")) {
                server_log(DS4_LOG_DEFAULT,
    ...
    if (c.max_slots > 1 && c.engine.mtp_path) {
        server_log(DS4_LOG_DEFAULT, "ds4-server: --mtp is not supported with --max-slots > 1 in batch mode");
        exit(2);
    }
    return c;
}
...
int main(int argc, char **argv) {
    ...
    if (cfg.max_slots > 1) {
        ds4_context_memory m = ds4_context_memory_estimate(cfg.engine.backend, cfg.ctx_size);
        server_log(DS4_LOG_DEFAULT,
                   "ds4-server: multi-slot session buffers estimated %.2f MiB total for %d slots",
                   ((double)m.total_bytes * (double)cfg.max_slots) / (1024.0 * 1024.0),
                   cfg.max_slots);
    }
    ...
    if (cfg.max_slots > 1) {
        if (ds4_batch_create_with_backend(&s.batch, engine, cfg.ctx_size,
            ...
            server_log(DS4_LOG_DEFAULT, "ds4-server: failed to create Metal batch runtime");
    ...
    if (s.session && s.kv.enabled && tokens && tokens->len >= s.kv.opt.min_tokens) {
        server_log(DS4_LOG_KVCACHE,
                   "ds4-server: persisting current KV cache before shutdown tokens=%d",
                   tokens->len);
        kv_cache_store_current(&s, "shutdown");
    } else if (s.batch && s.kv.enabled) {
        for (int slot = 0; slot < s.max_slots; slot++) {
            tokens = ds4_batch_tokens(s.batch, slot);
            if (!tokens || tokens->len < s.kv.opt.min_tokens) continue;
            if (!ds4_batch_slot_can_save(s.batch, slot)) continue;
            server_log(DS4_LOG_KVCACHE,
                       "ds4-server: persisting batch slot KV cache before shutdown slot=%d tokens=%d",
                       slot,
                       tokens->len);
            kv_cache_store_batch_slot_prefix(&s, slot, tokens, tokens->len, "shutdown");
        }
    }
    server_close_resources(&s);
    return 0;
...
static void test_responses_namespace_tool_schemas_restore_wire_namespace(void) {
    ...
    TEST_ASSERT(schemas && strstr(schemas, "\"type\":\"namespace\"") == NULL);
    const tool_schema_order *order =
    ...
    tc.arguments = xstrdup("{\"recency\":7,\"query\":\"deepseek v4 quants\"}");
    tool_calls_push(&calls, tc);

    buf out = {0};
    append_responses_output(&out, "resp_ns", "", &calls, &orders);
    ...
    TEST_ASSERT(strstr(out.ptr, "\\\"query\\\":") != NULL);
    TEST_ASSERT(strstr(out.ptr, "\\\"recency\\\":") != NULL);
...
static tool_schema_orders make_tool_search_order(void) {
    tool_schema_orders orders = {0};
    tool_schema_orders_add_json(&orders,
...
static void test_responses_input_loads_tool_search_output_tools(void) {
    const char *json =
        "["
...
static void test_responses_output_sends_tool_search_call_item(void) {
    ...
    tool_calls_push(&calls, tc);
    tool_schema_orders orders = make_tool_search_order();

    buf out = {0};
    append_responses_output(&out, "resp_search", "", &calls, &orders);
    ...
    TEST_ASSERT(strstr(out.ptr, "\"arguments\":{\"query\":\"perplexity\",\"limit\":3}") != NULL);
    TEST_ASSERT(strstr(out.ptr, "\"type\":\"function_call\"") == NULL);
...
static void test_responses_live_stream_sends_tool_search_call_item(void) {
    ...
    r.think_mode = DS4_THINK_NONE;
    r.has_tools = true;
    r.tool_orders = make_tool_search_order();

    responses_stream st;
    TEST_ASSERT(responses_sse_start_live(sv[0], &r, "resp_search_stream", &st));
    const char *raw_complete =
        DS4_TOOL_CALLS_START "\n"
        DS4_INVOKE_START " name=\"tool_search\">\n"
        DS4_PARAM_START " name=\"query\" string=\"true\">perplexity" DS4_PARAM_END "\n"
...
static void test_batch_options_parse(void) {
    ...
    c = parse_options((int)(sizeof(default_argv) / sizeof(default_argv[0])), default_argv);
